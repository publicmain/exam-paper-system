    ...
    <Screen>
      ...
      <section className="mt-4 grid gap-4 lg:grid-cols-[1.05fr_.95fr]">
        <Card>
          ...
          <p className="mt-1 text-sm leading-6 text-slate-600">每天推 10 个你还没学过的新词。学完之后，当天会多一份单词小测，什么时候做都可以。</p>
          {!today && !isTeachingDay() ? (
            <p className="mt-5 rounded-xl bg-slate-100 px-4 py-3 text-sm text-slate-600">{WEEKEND_VOCAB_NOTE}。想练的话可以用下面的抽查。</p>
          ) : (
        ...
        <Card>
          ...
          <div className="mt-4 grid grid-cols-4 gap-2">
            {([5, 10, 20, 'all'] as const).map((count) => <button key={count} aria-pressed={testCount === count} onClick={() => setTestCount(count)} className={`hit rounded-xl border text-sm ${testCount === count ? 'border-blue-600 bg-blue-600 font-medium text-white' : 'border-slate-200 bg-white'}`}>{count === 'all' ? '全部' : count}</button>)}
          </div>
      ...
      {overview.pendingTests.length ? <Card><h2 className="text-lg font-semibold">还没做的单词小测</h2><p className="mt-1 text-sm text-slate-600">没有截止时间。几天没做，每一天的都单独留着。</p><div className="mt-4 grid gap-2 sm:grid-cols-2">{overview.pendingTests.map((task) => <button key={task.dailySessionId} disabled={busy} onClick={() => void openPendingTest(task)} className="app-secondary flex min-h-[58px] items-center justify-between px-4 text-left"><span>{taskDate(task.date)} · {task.total} 题</span><span className="text-[#007aff]">{task.status === 'in_progress' ? '继续' : '开始'} →</span></button>)}</div></Card> : null}

      <Card>
        <div className="grid gap-3 sm:grid-cols-[1fr_auto] sm:items-end"><div><h2 className="text-lg font-semibold">查词并自由选择</h2><p className="mt-1 text-sm text-slate-600">查了不会自动加进生词本，加不加由你决定。和阅读时点词查到的是同一本词典。</p></div><div className="flex min-w-0 gap-2"><input value={dictionaryQuery} onChange={(event) => setDictionaryQuery(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') void lookUp(); }} placeholder="输入英文单词" className="min-h-[48px] min-w-0 flex-1 border border-slate-200 bg-white px-4 sm:w-72" /><button className="app-primary px-5" disabled={!dictionaryQuery.trim() || busy} onClick={() => void lookUp()}>查询</button></div></div>
        {dictionaryResult ? <div className="mt-4 rounded-2xl bg-slate-50 p-4"><div className="flex flex-wrap items-baseline gap-2"><span className="text-2xl font-semibold">{dictionaryResult.headword}</span><span className="text-sm text-slate-500">{formatPhonetic(dictionaryResult.phonetic)}</span></div><p className="mt-2 whitespace-pre-wrap">{posPrefixFor(dictionaryResult.pos, dictionaryResult.translation)}{cleanTranslation(dictionaryResult.translation)}</p>{cleanDefinition(dictionaryResult.definition) ? <p className="mt-1 whitespace-pre-wrap text-sm text-slate-500">{cleanDefinition(dictionaryResult.definition)}</p> : null}{dictionaryDone ? <p data-testid="dictionary-done" className="mt-4 rounded-xl bg-emerald-50 px-4 py-3 text-sm text-emerald-800">{dictionaryDone === 'learn' ? '✓ 已加入我的单词' : dictionaryDone === 'later' ? '✓ 已收进我的单词，稍后再学' : '✓ 已标记为会'}<button className="ml-3 underline" onClick={() => { setDictionaryResult(null); setDictionaryQuery(''); setDictionaryDone(null); }}>查下一个</button></p> : <><div className="mt-4 grid grid-cols-2 gap-2 lg:grid-cols-4"><button className="app-primary" disabled={busy} onClick={() => void lookUp('learn')}>加入我的单词</button><button className="app-secondary" disabled={busy} onClick={() => void lookUp('known')}>我已经会了</button><button className="app-secondary" disabled={busy} onClick={() => void lookUp('later')}>稍后再学</button><button className="app-secondary" disabled={busy} onClick={() => { setDictionaryResult(null); setDictionaryQuery(''); }}>只查一下</button></div><p className="mt-2 text-xs leading-5 text-slate-500">加入 = 不认识，现在开始学，复习会优先安排；稍后再学 = 先收着，以后再开始学；我已经会了 = 记下来，以后不当新词推；只查一下 = 什么都不记。</p></>}</div> : null}
      </Card>
      ...
      <Card>
        ...
        {data.items.length ? <ul className="mt-3 divide-y divide-slate-100">{data.items.map((item) => <li key={item.studentSenseId} className="py-4 lg:grid lg:grid-cols-[.8fr_1.25fr_auto] lg:items-center lg:gap-5"><div><span className="text-lg font-semibold">{item.headword}</span> <span className="text-sm text-slate-400">{formatPhonetic(item.phonetic)}</span><p className="text-sm text-slate-700">{posPrefixFor(item.pos, item.translation)}{cleanTranslation(item.translation)}</p></div><div className="mt-2 text-sm leading-6 text-slate-500 lg:mt-0"><p>{item.context?.sentence || item.definition}</p>{item.context?.translation ? <p>{item.context.translation}</p> : null}<p className="mt-1 text-xs">{SOURCE_LABEL[item.source] ?? '我的单词'}</p></div><div className="mt-3 flex items-center gap-2 lg:mt-0 lg:justify-end">{item.inNotebook ? confirmRemove === item.senseId ? <><button className="app-secondary px-3 text-sm text-rose-600" disabled={busy} onClick={() => void setMembership(item.senseId, false)}>确认移出</button><button className="hit px-2 text-sm text-slate-500" onClick={() => setConfirmRemove(null)}>取消</button></> : <button className="hit px-3 text-sm text-blue-600" onClick={() => setConfirmRemove(item.senseId)}>我会了，移出</button> : <button className="app-secondary px-3 text-sm text-blue-600" disabled={busy} onClick={() => void setMembership(item.senseId, true)}>重新学习</button>}</div></li>)}</ul> : <p className="mt-5 rounded-2xl bg-slate-50 px-4 py-5 text-sm text-slate-500">{query.trim() ? `没找到包含 “${query.trim()}” 的单词。` : '这个范围还没有单词。'}</p>}
      </Card>
    </Screen>
  );
}
