...
describe('背一背', () => {
  it('今天的词学完了 → 停在背诵页，**不自动开考**', async () => {
    ...
    expect(await screen.findByText('先自己默一遍，再去考试')).toBeInTheDocument();
    // 一个开考请求都没发出去
  ...
  it('中文默认盖住，点一下才露出来', async () => {
    await mount();
    await screen.findByText('先自己默一遍，再去考试');
    // 英文一直在
  ...
  it('点了「我背好了」才开考', async () => {
    ...
    await userEvent.click(screen.getByText('我背好了，开始考试'));
  ...
  it('「稍后再学」的词不进背诵单 —— 它也不会被考', async () => {
    ...
    await screen.findByText('先自己默一遍，再去考试');
    expect(screen.queryByText('induce')).toBeNull();
    expect(screen.getByText('3 个词')).toBeInTheDocument();
  });

  it('明说考试会抽查以前学过的词 —— 别让学生觉得考了没学的', async () => {
    await mount();
    const box = await screen.findByText(/抽查 3 个你以前学过的词/);
    expect(box).toBeInTheDocument();
  });

  it('全都点了「稍后再学」→ 没得考，说清楚而不是给一个开不了的按钮', async () => {
    await mount([{ headword: 'induce', translation: '引起', status: 'skipped', action: 'skip' }]);
    expect(await screen.findByText('今天的词处理完了')).toBeInTheDocument();
    expect(screen.queryByText('我背好了，开始考试')).toBeNull();
  });
  it('考完之后再回到这一页 → 送回首页，不再给一个「开始考试」的按钮', async () => {
    const calls: string[] = [];
    const fetchMock = vi.fn((url: string) => {
      const path = pathOf(url);
      calls.push(path);
    ...
    vi.stubGlobal('fetch', fetchMock);
    render(<MemoryRouter><VocabularyCoachLearnPage /></MemoryRouter>);
    await screen.findByText(/正在按你的进度准备词汇/);
    // 背一背不出现；也没有偷偷去开考
    expect(screen.queryByText('先自己默一遍，再去考试')).toBeNull();
    expect(calls.filter((p) => p === '/vocab-v2/test/start')).toHaveLength(0);
  });
});
