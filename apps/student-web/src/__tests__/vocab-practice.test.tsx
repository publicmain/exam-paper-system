...
describe('旧复习入口退役', () => {
  ...
  it('/vocab/practice 只作为兼容地址跳回统一中心', async () => {
    render(<MemoryRouter initialEntries={['/vocab/practice']}><App /><Probe /></MemoryRouter>);
    await act(async () => { for (let i = 0; i < 12; i += 1) await Promise.resolve(); });
    expect(screen.getByTestId('loc')).toHaveTextContent('/vocab');
    expect(fetch).not.toHaveBeenCalledWith(expect.stringContaining('/vocab/due'), expect.anything());
  });
});
