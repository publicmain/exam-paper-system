...
/** Exactly one lazy renderer, never loaded by a collection grid. */
export function MedalViewer({assetId,locked=false,reveal=false,onRevealComplete,onError,title,height}:{assetId:string;locked?:boolean;reveal?:boolean;onRevealComplete?:()=>void;onError?:()=>void;title?:string;height?:number}) {
 ...
 const callbacks=useRef({onRevealComplete,onError});callbacks.current={onRevealComplete,onError};
 const completed=useRef(false);
 const initialReduced=useRef(reduced);
 const playing=reveal&&!locked;
 const src='/medals/v5/viewer.html?'+new URLSearchParams({id:assetId,locked:locked?'1':'0',reveal:playing?'1':'0',reduced:initialReduced.current?'1':'0'});
 ...
 useEffect(()=>{
  ...
  const fail=()=>{setPhase('error');callbacks.current.onError?.();};
  ...
  const receive=(event:MessageEvent)=>{
   ...
   if(event.data.status==='ready'){window.clearTimeout(timeout);setPhase('ready');send('reduce',reducedRef.current);}
   else if(event.data.status==='error'){window.clearTimeout(timeout);fail();}
   else if(event.data.status==='complete')complete();
 ...
 return <div data-testid="medal-viewer" className="rounded-group bg-award-surface pb-1 text-award-ink">
  <div className="relative min-h-48 w-full overflow-hidden rounded-group" style={{height:height??'min(46dvh, 480px)',background:'rgb(var(--c-award-surface))'}} data-testid="medal-stage">
   {(phase!=='ready'||staticOnly)&&<MedalImage assetId={assetId} locked={locked} className="mx-auto h-full w-full object-contain"/>}
   {!staticOnly&&phase!=='error'&&<iframe key={src+':'+attempt} ref={frame} src={src} title={(title??medalName(assetId))+' · '+(locked?'未解锁灰色三维':playing?'颁奖动画':'三维藏品')} className={'absolute inset-0 h-full w-full border-0 '+(phase==='ready'?'':'invisible')} sandbox="allow-scripts allow-same-origin" referrerPolicy="no-referrer" onLoad={()=>send('reduce',reducedRef.current)} data-testid="medal-3d-frame"/>}
   {phase==='loading'&&<span className="absolute inset-x-2 bottom-2 text-center text-xs text-award-muted" role="status">正在呈现三维细节</span>}
  </div>
  {phase==='error'&&!staticOnly&&<div className="mt-2 px-3 text-footnote" role="status">已改为静态展示，收藏不会受影响。<Button className="!text-award-ink hover:!bg-award-muted/15 active:!bg-award-muted/20" size="sm" variant="plain" onClick={()=>{setPhase('loading');setAttempt(n=>n+1);}}>重试三维</Button><Button className="!text-award-ink hover:!bg-award-muted/15 active:!bg-award-muted/20" size="sm" variant="plain" onClick={()=>setStaticOnly(true)}>只看图片</Button></div>}
