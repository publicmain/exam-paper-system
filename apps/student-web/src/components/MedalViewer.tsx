...
export function MedalImage({ assetId, locked=false, thumbnail=false, className='', alt, onReady, onAnimationEnd, loadTimeoutMs }: { assetId:string; locked?:boolean; thumbnail?:boolean; className?:string; alt?:string; onReady?:()=>void; onAnimationEnd?:()=>void; loadTimeoutMs?:number }) {
  ...
  useEffect(()=>{
    ...
    const timer=window.setTimeout(()=>{if(loadedAsset.current!==assetId)setFailed(true);},loadTimeoutMs);
    return()=>window.clearTimeout(timer);
  },[assetId,loadTimeoutMs]);
  useEffect(()=>{if(failed||!medalByAssetId(assetId))readyCallback.current?.();},[failed,assetId]);
  const label=alt??medalName(assetId);
  const animationEnd=(event:AnimationEvent)=>{if(event.animationName==='award-flip')onAnimationEnd?.();};
  if(failed||!medalByAssetId(assetId)) return <div className={'grid place-items-center '+className} role="img" aria-label={label} onAnimationEnd={animationEnd}>✧</div>;
  return <img src={medalImage(assetId,thumbnail)} alt={label} width={thumbnail?320:880} height={thumbnail?320:880} loading={thumbnail?'lazy':'eager'} decoding="async" onLoad={()=>{loadedAsset.current=assetId;readyCallback.current?.();}} onError={()=>setFailed(true)} onAnimationEnd={animationEnd} className={className+(locked?' grayscale opacity-50':'')} />;
}
/** One lazy renderer. The collection remains interactive; ceremonies are presentation-only. */
export function MedalViewer({assetId,locked=false,reveal=false,ceremony=false,hold=false,fallbackRequested=false,onReady,onRevealComplete,onError,title,height}:{assetId:string;locked?:boolean;reveal?:boolean;ceremony?:boolean;hold?:boolean;fallbackRequested?:boolean;onReady?:()=>void;onRevealComplete?:()=>void;onError?:()=>void;title?:string;height?:number}) {
 ...
 const spinAfter=(delay:number)=>{
  ...
  startTimer.current=window.setTimeout(()=>send('start'),delay);
 };
 ...
 useEffect(()=>{
  completed.current=false;setSpun(false);setPhase('loading');setFallbackReady(false);
  let ready=false,failed=false;
  const fail=()=>{if(failed)return;failed=true;window.clearTimeout(startTimer.current);pending.current=null;setPhase('error');callbacks.current.onError?.();};
  failActive.current=fail;
  ...
  return()=>{failActive.current=()=>{};window.clearTimeout(timeout);window.clearTimeout(startTimer.current);pending.current=null;window.removeEventListener('message',receive);};
 },[src,attempt]);
 useEffect(()=>{if(fallbackRequested)failActive.current();},[fallbackRequested,src,attempt]);
 useEffect(()=>{
  if(!playing||!(phase==='error'||staticOnly))return;
  if(ceremony&&(hold||!fallbackReady))return;
  // CSS ends after the visible fade-in (0.75s) and one turn (1.6s).
  // animationend is authoritative; the timer only covers lost animation events.
  const timer=window.setTimeout(complete,ceremony?(reduced?0:2600):1200);return()=>window.clearTimeout(timer);
 },[phase,staticOnly,playing,assetId,ceremony,hold,fallbackReady,reduced]);
 return <div data-testid="medal-viewer" data-spun={ceremony?String(spun):undefined} className={ceremony?'award-medal-viewer':'rounded-group bg-award-surface pb-1 text-award-ink'}>
  <div className={ceremony?'award-medal-stage':'relative min-h-48 w-full overflow-hidden rounded-group'} style={ceremony?undefined:{height:height??'min(46dvh, 480px)',background:'rgb(var(--c-award-surface))'}} data-testid="medal-stage">
   ...
   {(ceremony?(phase==='error'||staticOnly):(phase!=='ready'||staticOnly))&&<MedalImage key={assetId} assetId={assetId} locked={locked} loadTimeoutMs={ceremony?8000:undefined} onReady={()=>setFallbackReady(true)} onAnimationEnd={()=>{if(!hold&&fallbackReady)complete();}} className={'mx-auto h-full w-full object-contain'+(ceremony&&playing&&!hold&&fallbackReady&&!reduced?' award-medal-flip':'')}/>}
