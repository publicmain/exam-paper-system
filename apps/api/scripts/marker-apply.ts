...
/**
 * ── 评语怎么写（2026-08-13 老师退回重写后定的规矩）──
 *
 * 原来的评语长这样，老师的评价是「一点都不像经验丰富的英语老师写的，
 * 学生根本看不懂」：
 *
 *     填11:patchwork,判对。1。**这道题出得有问题,不是你的错**:原文写的是…
 *
 * 四个毛病：①「填11:xxx,判对。1。」是给自己看的记账，题号/学生答案/
 * 判定/分数卡片上全都显示了；② markdown 的 ** 在网页上原样变成星号；
 * ③ 通篇解释「我为什么这么判」而不是「你该怎么做」；④ 没有可迁移的
 * 方法，学生下次还是不会。
 *
 * 现在的规矩：
 *   1. 对学生说话，不写记账，不用 markdown；
 *   2. 先说这题在考什么，再说他的答案差在哪；
 *   3. 末尾给一条下次用得上的具体做法（怎么定位、怎么排除、怎么检查），
 *      而不是只报答案；
 *   4. 英文关键词保留原文原词 —— 那正是学生要认得的东西；
 *   5. 答对也写一句，说清楚他做对了什么，别只有错题有反馈。
 *
 * 前端还有一层 cleanComment() 兜底，会剥掉历史评语里的记账前缀和星号，
 * 但那是补救，新写的评语不该再依赖它。
 */
