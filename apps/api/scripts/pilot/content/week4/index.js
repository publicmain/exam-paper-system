/**
 * 第四周（2026-09-21 起）内容包的装配。
 *
 * 从这一周起**全部原创**：题库里「从没发给学生过」的文章在首发周用完了。
 * 每个档位模块用 `../authored.js` 把一整天写明，这里只注入词表和例句中文：
 *
 *   · 词表来自 `vocab.generated.json`
 *     （`node ../../build-week2-vocab.js --week=week4 --csv <ecdict.csv>`）；
 *   · 例句中文来自 `../context-translations-week4.js`
 *     （`build-week2-context-translations.js --week=week4`，Azure，全自动）。
 *
...
const contextTranslations = require('../context-translations-week4.js');
...
function wordsFor(level, source) {
  ...
  if (!rows || !rows.length) throw new Error(`缺词表：${level} / ${source}（先跑 build-week2-vocab.js --week=week4）`);
  return rows.map((w) => {
