...
export class PdfDispatcherService {
  ...
  private async persistResult(f: SourceFile, resp: ProcessPdfResponse): Promise<void> {
    ...
    for (const p of resp.pages) {
      ...
      // PyMuPDF occasionally returns extracted text with embedded NUL
      // bytes (\x00) when the source PDF uses non-standard encoding.
      // Postgres TEXT columns reject NUL with error 22021 ("invalid
      // byte sequence for encoding UTF8: 0x00"), making the whole
      // file fail processing. CIE 9709 math papers in particular hit
      // this. Strip them — they carry no meaning for downstream
      // splitting / search and the surrounding text is preserved.
      const cleanText = p.text ? p.text.replace(/\x00/g, '') : p.text;
