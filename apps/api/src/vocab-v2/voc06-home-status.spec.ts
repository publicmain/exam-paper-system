...
describe('首页三项任务的后端事实（UI13 / UI14）', () => {
  ...
  it('阅读被系统收卷 + 学词完成 + 测试交卷 → 全部完成（收卷的阅读学生做不了，算结束）', async () => {
    const { db, stu, svc } = setup();
    const readingId = seedReading(db, '2026-09-07');
    const sub = db.seed('studentSubmission', { assignmentId: readingId, studentId: stu, status: 'submitted', finalSubmittedAt: new Date('2026-09-07T15:59:00.000Z'), submitSource: 'system_eod' });
    db.seed('answerScript', { submissionId: sub.id });
    const { last } = await learnAll(svc, stu, MON);
    const test = await svc.testSession(stu, last.generatedTestId);
    ...
    const overview = await svc.overview(stu, MON);
    expect(overview.home.reading).toEqual(expect.objectContaining({ state: 'auto_closed' }));
    expect(overview.home.allDone).toBe(true);
  });

  it('阅读还没发布 + 学词完成 + 测试交卷 → 全部完成；学词做完但测试没交 → 不是全部完成', async () => {
    const { stu, svc } = setup();
    const { last } = await learnAll(svc, stu, MON);
    expect((await svc.overview(stu, MON)).home.allDone).toBe(false);
    const test = await svc.testSession(stu, last.generatedTestId);
    for (const item of test.items) await svc.answerTestItem(stu, test.id, item.id, 0);
    await svc.submitTest(stu, test.id);
    const overview = await svc.overview(stu, MON);
