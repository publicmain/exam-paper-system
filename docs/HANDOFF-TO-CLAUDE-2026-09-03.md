...
- **词表生成器会选错词、挑错义项。** 它按词典查词形：人名地名只要词典里有同形词就当生词
  （Woodland →「林地」、Fleming →「佛兰芒人」），`sometimes` 被还原成 `sometime`（改天），
  多义词取的是 ECDICT 第一条义项（a full bottle 的 full →「把衣服缝得宽松」）。
  **但这些文章词目前不显示给学生**（2026-09-17 核实）：每日学词走新版词库、按档位词表推，
  文章词建出来的 StudentWord（`p1_w_`）「教过 / 复习过」全是 0，旧版学词页 LessonVocab 已不在路由里；
  学生卡片的释义来自新版词库的 VocabularySense，发布时线上词典 DictEntry（整本 ECDICT，
  没有试点词条）一律「保留原样」。所以下面两份表修的是**内容包本身的准确性**，不改变学生看到的东西。
  每周目录可放两份表，`build-week2-vocab.js` 读到才启用（第三周只放了义项表，因为已发布、不能换词）：
...
- **外部审查（2026-09-17，发布后、开课前）又抓到五类，已改并用 `--revise-unstarted` 重发 09-21~25：**
  1. 评分标准里写了原文推不出的结论（The Voice Message Q9「她没有骗作者」—— 原文只说照片是旧的）。
     **得分点必须是原文能直接支持的话**，写 rubric 时逐条问「哪一句能证明」。
  2. 科学表述错误（The Burr on the Dog：轨道上「没有重力」）。正文、参考答案、evidence 是同一句时要一起改。
  3. 概括题（`summary: true`）按意思给分，解析却说「原文在这个位置用的词是……」。`authored.js` 已改：
     第四周起概括题一律显示「参考答案 + 按意思给分」；更早的只在答案是改写时才换（第三周未受影响）。
  4. 情绪配对题的答案要有**叙述者自己的反应**作依据（表情、动作、心里话），「场景好笑」不等于「他觉得好笑」。
     第四周补了七句（Stamp 第 8 段；Voice Message 第 4 / 6 / 7 段；Egg Drop 第 3 / 5 / 7 段），
     补完再盲做一轮，确认每个时刻**在本段内**就有依据。
  5. 判断题不再要求每天 T / F / NG 各一（学生答出两道就能排除出第三道）。门禁（`publish-gates.js`）只拦「全部相同」，
     内容测试改为按周查：每种答案至少占五分之一，且第四周起不能天天各一。改答案要改题干，不能只换字母。
  盲做记录要绑定版本：`.local/reports/` 里存提交号、题目 ID、答案文本和结果，不能只存字母。
