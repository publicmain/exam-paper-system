...
2. **周一早上要不要重跑一次发布**。`StudentWord` 行只给「跑的那一刻已在班里」
   的学生建；真学生周一才注册，所以他们没有这批行。当天的 12 个主词来自
   `Paper.config.lessonWords`（不再依赖 `due` 队列），按代码注释这不影响上课；
   但这一条**没有用真账号验证过**，上线前应在走查里专门确认。发布脚本是幂等的，
   真要保险，周一学生注册后重跑一次 `--day=2026-09-07` 即可。
