...
- 数量变了就说出来：「学完了 decline。学完 4 · 延后 1 · 还剩 5」「decline 记为已会，换成 evidence。今天的总数不变」。
- 不知道就说不知道：模块没加载出来写「没加载出来，不代表没有」，不把未知当成零或「全部完成」。
- 只是放回集合写「重新加入」；只有真的开始教学才写「重新学习」。
...
- 性能预算（本机可测）：学生端首包 JS gzip ≤ 170 kB、CSS gzip ≤ 10 kB。2026-09-11 构建：JS 163.34 kB（基线 140.94 kB）、
  CSS 8.58 kB（基线 8.15 kB）。模糊只用在顶栏 / 标签栏的 `material-bar`，内容面不叠阴影不模糊。
  帧率、真机滚动流畅度、API 延迟没有测，不宣称。
