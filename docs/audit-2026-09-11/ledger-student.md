...
- 视口矩阵：`dev-fixtures/shoot.mjs`，34 个页面场景 × 10 个宽度（320 / 390 / 430 / 640 / 744 / 768 / 820 / 1024 / 1180 / 1366）
  × 浅深两套 = **680 张：横向溢出 0、小于 44px 的可点元素 0、页面报错 0**（最终代码 `daf6b87`）。
  **更正**：当时的溢出判定是 `scrollWidth > clientWidth + 1`，放过了 1px。上线后冒烟在手机宽阅读页量到 1px 横向溢出
  （`DraggableSplit` 吸顶条的 `-mx-px`）；已删，并把判定改成零容差，重扫 136 张 0 溢出，反证见 LEDGER「零容差复测」。
  矩阵先后抓到并修掉两处：320px 学习记录长徽标撑出 387px、「了解详情」行内链接 56×19（c6e604a）。
- 文字放大 200%（根字号，本项目字号全用 rem）：390 / 820 各 34 场景 = 68 张。修前 16 张横向溢出（今日 462px、
  学习记录 425px、我的单词 400px：页头把标题挤成一字一行、任务卡网格按内容撑宽、搜索框 12rem 最小宽随字号翻倍），
  修后 **0 溢出、0 小命中区**（daf6b87）。这是 Chromium 模拟，不是 iOS 动态字体。
- 性能预算（本机可测）：首包 JS gzip ≤ 170 kB（当前 163.39 kB，基线 140.94 kB）；CSS gzip ≤ 10 kB（8.60 kB）。
- 改前改后对照：`.local/audit/compare/index.html`（16 个页面，改前 = 6f85e6d）。
