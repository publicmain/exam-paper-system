...
1. **旧 DLC 的 `vocabWords = NULL` 按空集处理，但会在首次读取时自愈。**

   > ⚠️ **P7 实测更正**：这一条原来写的是「学生当天开不了正式测试，
   > 第二天自愈」。实际不是 —— P6 收尾把队列改成**每次读课程状态时与当前
   > 到期队列取并集**，所以学生（或教师看板）一打开，NULL 就被补成真实
   > 队列，当天就能考。实测：`dlc_stu_f` 的 NULL 在第一次
   > `GET /lesson/today` 之后变成 `["harbour","lantern","meadow","pebble"]`。
   >
   > 这比原来设想的更好（不用等一天），但**部署指导要跟着改**：见下面
   > 第 2、3 条的修订。空集只在「还没有人读过这一天的课程状态」的窗口里
   > 成立。
2. **部署仍应避开进行窗口，但理由变了。** 不是「当天考不了」（会自愈），
   而是**自愈时刻决定了队列内容**：在学生做完词、`due` 已被 FSRS 推远
   之后才补队列，补进来的是「此刻还到期的词」，未必是他上午真正做过的
   那批。**建议窗口仍是当天最后一个作答窗结束之后**（16:00–17:30 之后），
   那时补出来的队列已无人使用，第二天的任务行自带正确快照。
3. **部署前统计当日 NULL 且未完成的任务**，心里有数再动：
   ...
   数字不为 0 意味着这些学生的任务队列会在下一次读取时按**那一刻**的到期
   集合补齐 —— 不是考不了，而是考的范围由自愈时刻决定。阅读成绩、复习
   调度都不受影响。
