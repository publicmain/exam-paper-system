...
1. **旧 DLC 的 `vocabWords = NULL` 按空集处理。** 已有的当日任务行没有
   队列快照，出题时候选为空 → `insufficient_items`，学生当天开不了正式
   测试（会退回不计分的自由练习）。这是刻意选的安全方向：宁可当天不考，
   也不考不属于这次任务的词。
2. **部署应避开已有当日任务的进行窗口。** 早测窗口（08:00–09:00）和第二
   作答窗（16:00–17:30）内不要部署 —— 那时正有学生的 DLC 处于进行中，
   部署后他们的 `vocabWords` 仍是 NULL，当天的正式测试会缺席。
   **建议窗口：当天最后一个作答窗结束之后、次日零点之前**，这样第二天
   新建的任务行自带快照，自然自愈。
3. **部署前统计当日 NULL 且未完成的任务**，心里有数再动：
   ...
   数字不为 0 就意味着这些学生今天考不了正式测试。可以接受（他们的阅读
   成绩、复习调度都不受影响），但要知情。
