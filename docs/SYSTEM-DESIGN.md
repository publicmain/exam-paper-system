...
1. ~~首次 PIN 认领可被同班同学抢占（P0-1）~~ **已关闭，见 C.4**。
...
### C.4 集体注册窗口 + 教师只读学生视角（2026-08-25，`c6e6a37` / `4e9af48`）

关闭 C.3 遗留的 P0-1（首次认领可被抢占），**已上线生产并实测**。

#### 关键判断：风险取决于窗口，不取决于机制

一个直觉的方案是「让学生自己设密码，像注册流程一样」。这**不解决问题**：
互联网的注册之所以成立，是因为绑定了一个只有本人能收到的验证渠道
（邮箱 / 手机验证码）。学生没有这个渠道（`@school.local` 是合成邮箱），
所以「扫码 → 选名字 → 设密码」和「扫码 → 选名字 → 设 PIN」在身份证明上
是同一件事，换个词第一个到的人照样赢。

真正的变量是**认领窗口有多长、有没有人看着**。原设计窗口无限长：任何
未认领的名字，学期里任何一天都能被任何同学领走，被领的人可能几周后才
发现。

改成教师控制的短窗后，抢注要**当着老师和全班的面、在十几分钟里做，
而且当场就会被喊出来**。这不是密码学意义上的关闭，但把攻击成本抬到了
远高于收益的位置 —— 对一个班 35 人的日常学习系统，这是正确的性价比。

| 机制 | 说明 |
|---|---|
| `Class.pinClaimOpenUntil` | 班级窗（集体注册课）。教师开 10/20/45 分钟 |
| `User.pinClaimOpenUntil` | 个人补注册窗（请假 / 换手机 / 被抢注要重来）。**存在的理由是不必为一个学生重开全班窗** —— 重开会把所有未认领的名字重新暴露一次 |
| `setPin` 闸门 | 两个窗都关 → 403 `claim_window_closed`。**默认全关**，迁移后没有教师开窗谁都设不了 PIN |
| 一次性 | 认领成功即清空个人窗，不留给下一个人 |
| 时长上限 120 分钟 | 超了**抛错而不是静默截断**（教师以为开了 8 小时、学生说设不了 PIN，白排查一轮） |
| `/class-registration` | 投影用的操作台：大字号名单、5 秒自动刷新、名字当众变绿。**「当众变绿」本身就是防抢注的一环** |

上线时机是刻意的：生产 407 名在册学生此刻只有 1 个测试账号设了 PIN，
**真实分发还没开始**，正好赶在窗口打开之前把门装上。

#### 教师后台进入：只读，不是代登录

「老师从后台可以直接进入」有两种做法，差别很大：

- **(a) 以学生视角查看**（只读）
- **(b) 以学生身份登录**（可写）

选 (a)。(b) 的问题不是安全而是**数据可信度**：教师进去帮忙点两下，库里
记的就是「学生交了卷」「学生评了这个词」。判分队列和 FSRS 调度都建立在
「这条记录是学生自己产生的」之上 —— 一旦教师的动作能被记成学生的，
之后看任何一条记录都要先问「这是他自己做的吗」。排障需要的只是**看见**，
不需要代劳。

| 机制 | 说明 |
|---|---|
| `POST /student-auth/admin/view-token` | 签 `scope:'teacher_view'` 的 15 分钟令牌。沿用 `mq_handoff` 那套窄凭证模式，不新增机制 |
| `StudentIdentityGuard` | 读放行；凡 `@RequireStudentToken()` 的写一律 403 `teacher_view_is_read_only` |
| `set-pin` / `change-pin` | 也拒绝 —— 改凭证不能借视角做。教师要重置走 `admin/reset-pin`，那条路留痕 |
| sessionStorage | 令牌进**每标签页独立**的 sessionStorage，不碰 `localStorage.auth_token` —— 否则教师看完学生回到管理界面发现自己被挤下线 |
| URL 即抹 | 令牌从 URL 收下后立刻 `replaceState` 抹掉（历史记录 / Referer / 截图） |
| 常驻橙色横幅 | 教师视角和学生界面**长得一模一样**，不标出来会看混。横幅不可关闭，并明说只读 |
| `AuditLog` | 每次签发记 `teacher_view_student`（谁、什么时候、看了谁、什么 IP） |

#### 生产实测

- 窗口关着 `set-pin` → 403 `claim_window_closed`；开窗后 → 201；关窗后
  重复领 → 400 `pin_already_set`
- 教师端四个接口全通，超长开窗 → 400，审计三条动作全部落库
- 教师视角：读 `/me` 与生词本 200；写 `page-view` 与 `change-pin` 均
  403 `teacher_view_is_read_only`
- 浏览器实测 35 人花名册渲染、视角新标签页显示学生真实数据、横幅正确、
  「退出」隐藏、「修改 PIN」置灰、教师原标签页的登录态未受影响

**遗留的两项决策不变**：30 天 token 存 localStorage（P0-4）、读路径收口
的分阶段方案。

### C.5 4.0 每日一课 A0 + A 上线（2026-08-25，`346408d` 起）

按 PRD 自己定的阶段顺序做完：**A0 语义 → A 影子运行 → B 机制就绪但
默认关**。B 的前置是 A 跑满两个完整教学周，所以现在不打开。

#### A0：完成度必须能回答「今天到底学没学」

| 机制 | 说明 |
|---|---|
| `StudentSubmission.submitSource` | `student` / `teacher` / `system_eod`。**只有前两种计入完成度和连续天数**。收尾 cron 三处写入全标 `system_eod`（`eod_lock` / `window_close` / `stranded_draft_release`），课程页显示「已自动收卷」而非 ✓ |
| `DailyLessonCompletion` | target/progress/doneAt/source + `targetsFrozenAt` + `rulesVersion` + `autoFinalizeReason` |
| **目标冻结** | 学生首次打开课程页时写死当日三个目标。下午新增的错题进明天的课 —— 目标在学生背后往后退是最挫败的一种设计 |
| `rulesVersion` | 判定口径的版本。**变更时自动重新冻结当日目标**（只重算目标，进度不动）。不做这一步它就只是装饰 —— 加补段上限那次就是靠它让当天已冻结的学生立刻拿到新目标 |
| `effectiveLockAt` | `max(当天 23:59, 开卷 + 30 分钟)`，硬顶次日 01:00 |
| `Paper.rendererKey` | 渲染器不再从第一题推断（卷内词汇题上线后，第一题是什么纯属抽题顺序的偶然）。存量卷回退推断并打 warn |

#### A：课程页 + 完成度看板（只算不改）

- `GET /api/lesson/today`（学生，**会冻结目标**）
- `GET /api/lesson/class`（教师，**不冻结** —— 教师看一眼不能给全班
  创建当日记录，那会污染「学生今天来过」这个信号）
- `/my-lesson`：三段并列不强制顺序、写「通常 N 分钟」而非限时、
  未开始的段落不用红色、全绿只给一句话不发徽章
- `/lesson-board`：按完成度升序，一眼看到「三个 ○」的人；分数退居末列

**目标一律可达成**：背段 = `min(到期词数, reviewBatchSize)`；
补段 = `min(队列长度, 5)`；今天没有的段落算完成；分母恒为 3。

#### 上线过程中被生产实测抓到的四个 bug

文档价值最高的部分 —— 这些都是单测没覆盖、跑起来才现形的：

1. **日期差一天**。`MorningQuizSession.date` 存的是「SGT 日历日的 UTC
   午夜」（日期标签），我用了「SGT 真实零点」的算法（时间瞬刻），多减
   一次时区 → 下午查出来是昨天，读段恒为「今天没有安排文章」。已拆成
   `lessonDayKey` / `sgtMidnightInstant` 两个纯函数并补边界测试。
2. **一个班一天多场时挑错场次**。R10 多层每层一场，`findFirst` 随便挑
   一场 → 学生明明交了卷却显示「未开始」。改为优先认他真有答卷的那场。
3. **「0/20 道 · 约 20 分钟」**。补段漏了背段早就有的「可达成」上限。
   连带发现进度算法「目标 − 队列剩余」在封顶后恒为 0，改成直接数当天
   练过的错题条数。
4. **卷名把内部 setCode 摔到学生脸上**
   （`Morning Quiz OLEVEL/ai_authored_olevel_basic_05_the_queue_v1/…`）。
   加 `readablePaperTitle` → 《The Queue》，认不出来就不显示标题。

#### B：机制就绪，默认关

`MORNING_QUIZ_ALL_DAY` 支持 `on` / `off` / 班级白名单（灰度）。关着时
窗口取值与原来一字不差；打开则 00:00–23:59 且跳过第二窗自动开窗。
回滚是改环境变量，不用重新部署。
