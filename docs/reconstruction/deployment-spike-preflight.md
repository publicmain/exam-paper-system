...
> 的浏览器或 PWA。
>
> **阶段 3 已收口（2026-08-27）：COMPLETE WITH DEFERRED INTEGRATION CHECKS。**
> 拓扑定案为**方案 A（学生端独立源）**，见
> [D7](./product-decisions.md#d7--部署拓扑方案-a学生端独立源)。
> 余下的检查项**不再挂在阶段 3 名下**，已按归属重新分配到各自的阶段
> （见 §12）。**阶段 3 不再阻断阶段 4。**
...
### ✅ **定案（2026-08-27）：方案 A —— 学生端独立源**

见 [D7](./product-decisions.md#d7--部署拓扑方案-a学生端独立源)。下面三行是结论，
再往下是当初的推理过程（保留备查）。

| 方案 | 结论 |
|---|---|
| **A 独立源** | **采用** —— S2 已在 staging 实测通过（新源含深层路径全绿、既有服务零退化），S3A 又在真机 Safari 上确认了 SPA 兜底 |
| **B 同源反向代理** | **否决 —— 不必要的基础设施**。S1 证明平台不提供路径级转发（R8），B 必须自建代理 = 学生唯一入口前的新单点；而 A 已经够用 |
| **C 边缘重定向** | **可选，且现在不再调查**。它**只对整班全量切换有意义**，**不是阶段 4 的前提，也不是按学生 ID 灰度的前提**（Edge Rule 匹配不到学生身份）。S3A 还实测证明它**离线时根本不触发**。推迟到阶段 13/15 再评估 |

（以下为定案前的推理，保留备查。）

#### 定案前的推理：**A 为主**；**C 作为切换后的可选加速器（UNVERIFIED）**；**B 为最后退路**
...
| # | 状态 |
|---|---|
| **A1** 新服务能拿到主机名 | ✅ **已解决** —— S2 实测生成成功。**staging 就用这个生成域名**（D7）；**生产用专用自定义主机名，具体域名推迟到生产规划**（D7）。这不阻断阶段 4 —— 新端从 API 运行期取 `studentAppOrigin`，换域名不用重新构建 |
| **A2** API 的 `CORS_ORIGINS` 可追加新源 | 🔶 **未验证，已移交** —— 归**阶段 4 的 staging 集成出口条件**（原 S6） |
| **A3** 学生在新源重登一次可接受 | ✅ **已决定：可接受**（D7） |
| **A4** 套餐是否含 edge-rule allowance | ⏸ **非阻断，已推迟** —— 它只是方案 C 的前提，而 C 已推迟到阶段 13/15。**现在不查** |
...
> **收口说明（2026-08-27）**：O1 已排除（S2 实测拿到了主机名）；
> O3 移交阶段 4 验证；**O2 仍未被检验** —— 它依赖 M10，而 M10 缺测试
> 载体（§10.5）。**O2 因此成为方案 A 的一项显性残余风险，推迟到阶段
> 13/15**：那时真实的旧→新适配器已经存在，M10 会作为它的自然一部分
> 被验证，**不需要为此临时造测试载体**。
...
**S1、S2、S3A 已完成。** 其余各项**不再挂在阶段 3 名下** ——
已按归属重新分配（见 §12）。下面的定义保留为执行说明。
...
| 系统 | iOS 26.6 |
| 旧 staging PWA 初始状态 | **未安装**（OBSERVED —— 主屏上没有该图标） |
| 安装之前 Safari 里是否已有旧源的 SW 注册或缓存 | **UNVERIFIED** —— 见下方说明 |
...
> 基线如实记为「原本未装」。
>
> **一处必须说清的证据边界**：「主屏上没有图标」证明的是**PWA 未安装**，
> **不等于**「Safari 里没有旧源的 Service Worker 注册或缓存」。在普通
> Safari 标签页里访问过旧源，同样会注册 SW（`main.tsx` 的注册条件只有
> `import.meta.env.PROD`，与是否安装到主屏无关）。
> **安装之前该设备上是否已存在旧源的 SW 注册或缓存 —— UNVERIFIED**：
> 要确认它需要在 Safari 里检查站点数据，本轮没做（也不在本轮授权的
> 必要范围内）。
>
> 这个边界不影响 EV-03 的结论 —— EV-03 观察的是**安装之后**的状态，
> 那时旧 SW 确定活着（EV-02 已在线加载过一次并被缓存）。
...
| **EV-03** | **安装之后**，**离线**（飞行模式 + 强杀 + 重开）打开旧 PWA | **照常打开，渲染缓存里的同一个旧壳**（同一句「请从个人主页进入」），iOS 只叠了系统的飞行模式提示。**没有出现浏览器错误页** | **OBSERVED**（限定于「安装之后」）—— 关键格 |
...
## 12. 阶段 3 收口 —— 检查项的重新归属（2026-08-27）

**阶段 3 的目的**是「在写第一行页面代码之前，把部署形态弄清楚」。
这个目的**已经达到**：拓扑定案为 A，并在 staging 与真机上实测过。

余下的检查项**不属于「部署形态」这个问题**，把它们继续挂在阶段 3 名下
只会让一个已经回答完的问题永远显示未完成。按归属重新分配：

| 原编号 | 内容 | 状态 | **新归属** |
|---|---|---|---|
| **S1** | 路由能力 | ✅ PASS | 阶段 3（已完成） |
| **S2** | 独立源拓扑 | ✅ PASS | 阶段 3（已完成） |
| **S3A** | 真机设备观察 | ✅ 完成 | 阶段 3（已完成） |
| **M10** | 旧 PWA 内部跨源导航的 standalone 行为 | ⏸ **显性残余风险** | **阶段 13 / 15** —— 等真实的旧→新适配器存在之后随它一起验证。**不为此造临时测试载体** |
| **M6–M9** | 旧端更新、`unregister`、删缓存、回滚 | ⏸ 推迟 | **阶段 13** —— 那一阶段才第一次真正实现 SW 的退役与更新行为 |
| **S4** | 灰度判定层 | ✅ **设计已闭合** | 实现归**阶段 5 / 15** |
| **S5** | `STUDENT_APP_ORIGIN` 注入 | ✅ **设计已闭合** | 实现归**阶段 4** |
| **S6** | 跨源请求形态与 CORS | 🔶 移交 | **阶段 4 的 staging 集成出口条件** |
| **S7** | 夹具安全执行环境 | 🔶 移交 | **阶段 14 的硬前置**（播种之前必须满足）。**不阻断阶段 4** |
| **A4** | 套餐是否含 edge-rule allowance | ⏸ 非阻断 | 随方案 C 一起推迟到**阶段 13 / 15** |
| **U2** | 同一主机名能否挂两个服务 | UNKNOWN | **已无关** —— A 不需要它 |

### S4 设计闭合的内容

**API 在令牌认证之后按学生 ID 决定应用版本，fail-closed 到 v1。**

- `STUDENT_APP_V2` 是 **API 服务**的环境变量，启动时校验（照
  `assertAllDayConfig` 的硬门），**前端永不解析它**
- `login` / `me` 的回执带 `appVersion: 'v1' | 'v2'`
- 字段缺失、解析失败、请求失败 → **一律按 `v1`**（留在旧端）
- 回滚 = 改一个环境变量并重启，两个前端都不用重新部署

### S5 设计闭合的内容

**API 在运行期同时下发 `appVersion` 与 `studentAppOrigin`；
换 origin 不需要重新构建前端。**

这条直接解决了「生产主机名尚未确定」不阻断阶段 4 的问题。

### 阶段 3 出口判定

> **COMPLETE WITH DEFERRED INTEGRATION CHECKS**

- 部署形态问题**已回答**：方案 A，有 staging 与真机实测证据
- 推迟的每一项都**指名了新归属阶段**，没有一项悬空
- **M10 不记为已观察，S3 不记为全部验证** —— 它是显性残余风险，
  在阶段 13/15 由真实适配器验证
- **阶段 3 不再阻断阶段 4**

### spike 服务的处置

`stg-student-web-spike` **保持在线、保持不动**，供阶段 13 的集成测试
使用（那台测试 iPhone 上的旧 staging PWA 也**保留不删**，理由相同）。

**清理方式**（阶段 13/14 用完之后，或方案 A 被推翻时）：

1. 删掉该服务生成的那一个 Railway 域名
2. 删掉服务 `stg-student-web-spike`
3. 确认回到 `Postgres` / `stg-api` / `stg-web` 三服务状态
4. 仓库里的 `spikes/student-web-origin/` 可一并删除（它与产品代码零耦合）

它与既有服务**零耦合** —— 不共享变量、不连数据库、不走私有网络，
删除不影响任何东西。
