...
| **旧 SW 干扰** | **构造上不可能**（跨源） | 旧 SW 作用域 `/`（F6）会拦 `/app/*` 的导航与同源静态资源（F8），离线兜底还指向旧路由（F10） | **构造上不可能**（终点是跨源） |
| **既有书签 / 主屏图标** | 靠**应用层**跳转送走 | 继续有效 | 靠**边缘**送走 —— 比 A 更早、更可靠：**不用等 SPA 加载、不依赖 JS、旧 SW 也拦不住**（重定向发生在边缘） |
...
### [S1] 推荐：**A（独立源）**；**C 作为迁移期的加速器**；**B 降为最后退路**
...
2. **C 是新发现的、有价值的选项 —— 但它是 A 的补充，不是替代。**
   ...
   应用层挪到边缘**：不用等 SPA 加载、不依赖 JS、**旧 SW 也拦不住**
   （F6/F8 的干扰在重定向发生时还没机会介入）。这恰好命中 A 的最大痛点
   —— 已装 PWA 与旧书签的迁移（O2 的风险面）。
...
| **通过判据** | U1、U2 有确定答案 |
| **结果** | **PASS** —— **U1 RESOLVED**：平台**不支持**路径级多服务转发（R8），但**支持**路径匹配 + 重定向（R6/R7）。**U2 仍 UNKNOWN**，但**已不影响决策**（见下） |
| **回滚** | 无（纯读；勘察前后状态逐项一致） |
| **授权** | **C1 Railway 读** ✓ 已行使 |

> **U2 未解决为何不阻断 S1**：S1 的目的是回答「同源路径分流可不可行」。
> R8 已经从**动作层面**否定了它 —— 即便同一主机名能挂到两个服务（U2），
> 也没有任何 Edge Rule 动作能按路径把请求送到其中某一个。U2 的答案
> 不会改变拓扑结论。要严格闭合它需要**写操作（C4）**，C1 下做不了，
> 也不值得为此申请写权限。
...
**S2–S7 全部未完成。** 最小的下一步授权见 §11。

---

## 11. [S1] 下一步所需的最小授权

**S1 已闭合。下一件事有两个候选，按代价从小到大：**
...
- **授权类别**：**C2（写配置）+ C3（staging 部署）**；若要验方案 C 的
  Edge Rule，另需 **C4**
- **风险**：会在 staging 项目里新建一个服务
- **回滚**：§7.4 的拆除步骤

> **建议先只批候选一。** 它零风险，且它的答案会影响 S2 到底要验哪个
> 拓扑 —— 先把 A4 问清楚，S2 的设计才不会白做一遍。
