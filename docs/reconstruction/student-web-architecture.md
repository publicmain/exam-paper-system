...
- **前缀待定** —— 取决于 §7 的部署 spike 结论：独立域名下是 `/`，
  同域名双 SPA 下是 `/app`。下文一律写 `/app/…`，若走独立域名则
  整体去掉 `/app` 前缀，**页面清单与矩阵不变**。
...
- **不改**：规则 3（无令牌的读放行）在阶段 10 之前保留 —— 旧端还在用。
...
- [ ] **S6** 确认新端的 API 调用形态（同源相对路径 / 跨域 + CORS）

**退出条件**：staging 上两个空白页面同时可访问，旧 SW 环境下新端能
正确更新，且灰度开关能把一个学生 ID 路由到新端。
