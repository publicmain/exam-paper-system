...
## 阶段 4B2 —— 八账号生命周期 + 教师重置　⬜ **需授权**

- [ ] 八个 staging 账号逐个走通：登录 / 注册 / 改密码 / 退出
- [ ] **教师重置**：重置后学生被踢回登录页，且能重新注册
- [ ] 失败路径：密码错、锁定、同名消歧
- [ ] 浏览器里的问候语与登出清票（4B1 未做，见上）

**授权类别**：**C7**（要动 staging 数据库里的账号状态：注册、改密码、
教师重置都会写库）+ **C1**（对账）。改密码与教师重置会**撤销令牌**，
执行前要知道这会把在测设备踢下线。
