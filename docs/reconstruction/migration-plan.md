...
## 阶段 12 —— 生词本与错题本　🔧 **12A–12D 本地完成 · 12E staging 实机验证完成 · 12F 账号就绪 · 12G/12H/12I 首次真人验收失败，服务端与界面两半已改完 · 12J 夹具重建并部署 · 12K 重建计划 · 12L 试点最小改动上线 · 12M 真内容进库、试点开跑 · 12O 学生自助注册与自选难度 · 12P 五档全周内容与双端闭环上线**（2026-09-01），**整阶段 `PILOT_LIVE_FOR_SMALL_COHORT`**
...
**返工 1/2**（补词典 + 建账号）基线 `5ffe66b` · 提交 `dc13cf2`。
**返工 2/2**（重建 + 普通登录探针）基线 `47da846`。
词典已补、账号已建、当天已干净、普通登录已跑通
—— 见本节末尾的 **12F** 小节。
...
还有两项：`ExamWordSheet` 的 token-only 重写，以及**这五页的实机验证**
（到现在为止它们一次都没对着真 API 跑过）。
...
### 12B —— 错题本与错题重练　**本地完成**（2026-08-30）

`task_id: S12B-MISTAKE-BOOK-AND-RE-PRACTICE-LOCAL` · contract v1.0 ·
base `4d4ef87` · 实现提交 `4e5f3a9`。
**同样是纯本地任务：没有部署、没有 staging 执行、没有任何数据库读写。**

- [x] `/mistakes`、`/mistakes/practice`（去掉 `/app` 前缀，D7）
- [x] token-only：姓名 / studentId 不出现在 URL、查询串、请求体、存储里

#### 端点边界

| 路由 | 读 | 写 |
| --- | --- | --- |
| `/mistakes` | `GET /vocab/mistakes?includeResolved=1` | `POST /vocab/mistakes/resolve` `{id, resolved}` |
| `/mistakes/practice` | `GET /vocab/mistakes/practice-queue` | `POST /vocab/mistakes/practice-result` `{id, correct}` |

四条后端接口都走 `resolveIdentity()` → `identityOf()`，**带令牌就按令牌里的
id 精确查**。新端一个身份参数都不传；**唯一允许出现的查询串是
`includeResolved=1`** —— 它是**视图开关**（已销账的要不要一起给），
与「谁在问」无关。**后端一行没改。**

两页都不碰：`/lesson/*`、`/vocab/words*`、`/vocab/due`、`/vocab/review*`、
`/vocab/quiz*`、`history-by-name`、正式测试的 attempt、`page-view`、
旧端 `/my-*` 与 `/scan`。也不写任何 storage 键（守卫钉住）。

#### 错题本这一屏

**一次取全，前端分两段。** 分两次取会出现「上半屏的总数」和「下半屏的
列表」来自不同时刻的窗口 —— `total` 与 `byTaskType` 是服务端按**未销账**
算的。两段各自保留服务端顺序（按天倒序、同天按收录原因排）。

**已销账那一段必须在。** 错题本要能清空，否则只会一直变长；但「清空」
不能是删掉 —— 学生手滑点了「已弄懂」得能拿回来。所以销账是一个**可逆的
标记**，两段都摆在页面上，已销账那段带「放回」。

**字段照搬、缺的不显示**：题型、原因、篇目、日期、题干、当时的答案、
正确答案、得分/满分、老师评语、答案要点、范文、解析、原文依据、重练连胜。
空串就是「没有」，不编空评语、不编空解析。服务端文本一律当**纯文本**渲染
（守卫禁 `dangerouslySetInnerHTML`）。有 `submissionId` 才给成绩详情入口，
而且走既有的 `scoreDetailPath()` 编码。

**`{updated:0}` 是失败。** 服务端返回的是**受影响行数**：`0` 表示没有一行
匹配（不是我的、或者已经不在了）。把它当成「成功但没变化」，学生会看到
那条错题原地消失又回来。写成功之后**重新取一次权威列表**；对账失败时
**一个数字都不显示**，明说「刷新一下」。

#### 含糊的失败：先读回来，别盲目重发

`POST /vocab/mistakes/resolve` 本身是幂等的，但**「请求失败」不等于
「没写成功」** —— 响应丢了的那种失败，重发只是又猜一次。做法是先把列表
读回来：

  · 已经是目标状态 → **当成成功**（本来就成了），不再写第二次；
  · 还是老状态     → 让**学生自己**再点一次；
  · 读也失败       → 停在闭锁态，**不自动再写**。

#### 重练这一屏

**四种作答方式全支持**，选项一律**用服务端给的**：

| kind | 来源 | 判定 |
| --- | --- | --- |
| `tfng` | 固定三键 | 与 `correctAnswer` 比，只抹首尾空白与大小写 |
| `letters` | 从原文 `Paragraph X` 推出的字母 | 同上 |
| `options` | 题库里的完整选项（字符串或 `{key,text}`） | **按 `key` 比**，显示看 `text` |
| `reveal` | 主观题 | 先翻卡，再自评「还没掌握 / 我答对了」 |

判定只抹首尾空白与大小写。再宽一点就开始替学生判分了 —— 这一屏没有 AI、
也不该有；判宽了学生会以为自己掌握了，而这道题本来就是他错过的那一道。

**作答之前，答案材料一个字都不进 DOM。** 队列响应**本来就带着**正确答案、
要点、范文、解析、证据句 —— 这是自由重练不是考试，服务端不做遮挡
（`stripUnreleasedScores` 是考试那条链的）。**遮挡是这一屏的责任**：
做不到的话，「重练」就变成了「看一遍答案」。旧答案与老师评语同理，
只在反馈里出现。

**连胜与销账照搬服务端**（`correctStreak` / `resolved`），前端不自己推。

#### 这一阶段最要紧的一条：重练写入没有幂等键

`POST /vocab/mistakes/practice-result` **没有 `requestId`**，服务端每收到
一次就 `practiceCount + 1` 并重算连胜 —— 而「隔天连对两次自动销账」正是
靠这个数。**重发一次就等于多做对一次。**

所以这一屏的写入规矩比生词本那边更严：

  · 在途期间**不接受**下一题 / 跳过 / 再答；
  · 网络失败**绝不盲目重发**，改为**读回队列**再判断 ——
      这道题还在  → 那次没记上，给一个**学生自己点**的重试；
      已经不在了  → 那次记上了，给一个**不编数字**的回执
                    （连胜与销账只能来自服务端，猜一个出来比不说更糟）；
      读也失败    → 停在这一题，只给「再查一次」，**不自动再写**；
  · `{ok:false}` 是**服务端明说没记上** —— 留在原题，可重试。

跳过在作答之前照常可用（一个请求都不发），**写一旦开始就没有跳过了**。

守卫 G-12B 静态钉住「发送点只有一处」：`api.mistakePracticeResult(` 在
页面里**恰好出现一次**，`api.mistakeResolve(` 同理。有第二处，就一定会有人
在某条错误分支上「顺手再发一次」，而那正是会把连胜算歪的那件事。

#### RED（对着 base `4d4ef87`，行为红不是收集红）

```
npx vitest run src/__tests__/mistakes.test.tsx \
               src/__tests__/mistake-practice.test.tsx
→ exit 1 ·  Test Files 2 failed (2) ·  Tests 58 failed | 7 passed (65)
   mistakes         26 failed / 31
   mistake-practice 32 failed / 34
```

覆盖了合同点名的每一项：两条路由都没注册、Today 与今日总结没有入口、
列表那个 GET 没发、销账 / 恢复的控件不存在、四种作答方式一个都不渲染、
`practice-result` 的写入与对账行为不存在、401 处理不存在。

那 7 项通过的是「没票时不发请求」「不写存储」「卸载后不画」这类**否定
断言**，在不存在的页面上本来就成立 —— **不算**这次 RED 的判据。

#### GREEN（`4e5f3a9`）

| 命令 | exit | 结果 |
| --- | --- | --- |
| `mistakes` / `mistake-practice`（聚焦） | 0 | **31** / **34** |
| `contract.test.ts` | 0 | **147**（132 → +15：G-12B 整块 + 路由线一条） |
| `today` / `lesson-summary` | 0 | 29 / 25 |
| 12A 三份（**一行未改**） | 0 | 37 / 32 / 30 |
| `vitest run`（student-web 全量） | 0 | 25 files / **841** passed（761 → +80） |
| `npm run typecheck` / `npm run build`（student-web） | 0 | 干净 · 326.02 kB |
| `vitest run` + `typecheck`（api 全量） | 0 | 93 files / **1334** passed |
| `vitest run` + `typecheck`（web 全量） | 0 | 37 files / **247** passed |
| `git diff --check` | 0 | 无空白错误 |

**改动的文件**：新增 `pages/Mistakes.tsx`、`pages/MistakePractice.tsx`、
`components/mistakes/answer-check.ts` 与两份测试；修改 `App.tsx`、
`routes.contract.ts`、`lib/api.ts`、`Today.tsx`、`LessonSummary.tsx`、
`contract.test.ts`、`lesson-summary.test.tsx`。
`today.test.tsx` **不需要改**（它没有断言出口集合）。
`apps/api`、`apps/web`、Prisma、seed、依赖、Dockerfile、Railway 配置、
**12A 的页面与测试全部未动**。

**授权的配套测试改动**：注册路由集合十四条 → 十六条；`KNOWN_ENDPOINTS`
登记四条新端点；`lesson-summary.test.tsx` 里阶段 10 那条「不给还没实现的
出口」把 `/mistakes` 列为禁止项，现在它真的存在了 —— 规矩没变（**先有那
一页，才能有指向它的出口**；旧端路由永远不行），只是把它从禁止挪到允许。
G-12A 里那条「错题本」禁令的**规则一字未动**（生词本那一面仍然不许碰
错题端点），只更新了措辞里已经过时的「还没实现」。

#### 一处测试夹具的更正

`expectNoAnswerMaterial()` 一开始拿证据句的原文子串去断言「没泄漏」，
而**证据句按定义就是原文里的一句话，原文是该显示的** —— 那条断言分不清
「证据字段漏了」和「原文正常渲染」，两条测试因此红。夹具改成给证据句加一个
...
### 12F —— 仿生产的合成验收账号　**账号就绪，等用户走查**（2026-08-30，历经 v1.0 NO-GO 与两轮返工）
...
#### 返工 1/2 —— 补词典、建账号，然后在最后一道对账上停住（2026-08-30）

`contract_version: 1.1` · 基线 `5ffe66b` · 实现提交 `dc13cf2`。
用户逐条批准了**一项新的 staging 权限**：往 `DictEntry` 里插入**正好 51 行**
（50 个生词本用词 + 留给查词的 `blossom`），并明确说明**这不是**「以后可以
随便改词典」的通行证。

##### 词典：8 → 59，只插不改不删

`apps/api/scripts/staging/seed-s12f-dictionary.js`。
词典的主键就是单词本身 —— **带不了 `s12f_` 前缀**，所以「只碰自己的行」
这条纪律换了三种表达：

  1. **只插不改不删**：没有 upsert、没有 update、主执行路径里没有 delete
     （有测试扫源码盯着）；`createMany` 且 `skipDuplicates: false`
     —— 撞了就报错，绝不悄悄跳过（跳过等于默认「库里那行是对的」）。
  2. **一份写死在仓库里的 51 词清单**，确定性哈希
     `f6091e59acdf99847f580bd75b04d510b54aa770968c4e74a08ec1a48c9b0318`。
     每一行都有真音标、真中文释义（**以词性开头** —— 干扰项按
     `posOf()` 配对要用）、真英文释义；中英释义各自 51 条互不相同，
     没有占位符。
  3. **插入前三态判定**：全不在（首次）/ 全在且**逐字段相同**（幂等重跑）/
     **其余一律 NO-GO**。部分存在、内容不一致，都不做「补一补」——
     那是在共享数据上制造混合状态。

**九类反例在 staging 上逐个跑过，全部拒绝，且不回显任何取值**（含
「拿夹具的确认串来冒充」—— 词典插入有它自己的确认串）。

实测：`8 → 59`，**delta 恰好 +51**；原有八词的哈希**未变**；51 行**逐字节
相同**；库里没有第 60 行。

**回滚只写不跑**：`rollbackPreflight()` 算出「会删哪 51 个词」并把拦阻条件
查一遍（专用确认串、S12F 账号必须已不在、`s12f_` 资源必须已清、
不能有非 S12F 的生词 / 错题 / 测试快照引用这些词、库里内容必须与清单一致），
然后**停下**。返回值里 `executed` 恒为 `false`，全程零写语句。
原有那八个词**永远不在计划里**。

##### 账号：建出来了，历史数据全对

夹具跑了一次，退出码 0。只读对账把冻结的分布逐条打到真库上，
**59 项里 57 项通过**：

| 判据 | 实测 |
| --- | --- |
| 身份 | `s12f_acceptance_student` · 学生 · 在读 · 未归档 · `ielts_authentic` · `authVersion=0` |
| 凭据 | 有 `pinHash`，且**与 `passwordHash` 不同**；`lastLogin` 仍是 null |
| 班级 | 专属班、未归档、角色 `student`、班级分级 `ielts_authentic` |
| 阅读 | 12 份（判完 10 · 待判 2），12 个不同篇目，分数 0 / 4 / 4 / 5 / 5 / 5 / 5 / 5 / 6 / 8 |
| 逐题 | 72 条，**逐题得分之和 == 总分**且不超满分；有空白、有评语、待判卷仍有未判的主观题 |
| 课程 | 13 个历史任务行（完成 9 · 半途 4） |
| 正式测试 | 11 次，含 0 分与满分，**四种题型齐**，每次都挂在当天任务行上，今天 0 次 |
| 生词 | 50（新 10 / 学 12 / 复 20 / 掌握 8；重学 5），到期 21 / 将来 29，**50 个全部有词典音标+释义** |
| 复习流水 | 198 条，跨 14 天，四种评分 |
| 错题 | 20（未销账 16 · 已销账 4），6 种题型，三种收录原因齐，练习次数 0–3，连对 0 与 1 |
| 申诉 | 2 条，正文与批注都带 `STAGING SYNTHETIC` |
| 今天的卷子 | `rendererKey=ielts_reading` · `config.mode=passage_pick` · 10 题 · 4 种任务类型 · 作答窗 23.97 小时 |
| `blossom` | 在词典里 ✓ · 不在生词本里 ✓ · 在今天的原文里 ✓ · **不是任何一道题的答案** ✓ |
| 隔离 | t1–t8 的完整指纹**逐字节相同**；除本账号外仍是 8 个学生 |

##### 但是：**14 条复习流水落进了今天** → NO-GO

夹具把所有「N 天前」都按 `now − N×24 小时` 算 —— 那是个**相对此刻**的瞬刻。
实跑时刻是 **22:18 SGT**，于是「1 天前 + 3 小时」= 昨天 17:18Z，
而**今天的 SGT 零点是昨天 16:00Z**。14 条复习流水因此落在了今天。

> 这不是个记账小数点。学生还没动手，`/vocab` 就会显示「今天复习 14 次」，
> 而且 `dueQueue` 会把「今天复习过的词」也拉进今天的队列 —— 正式考试范围
> 当场变形。AC-09 要求当天**一条复习流水都不能有**，所以这条不算通过。
>
> 同一类问题在 `lastPracticedAt`（最近一次 1 天前）和 `lastReview` 上
> **只是侥幸没踩到**：它们比边界早了一个多小时。侥幸不是设计。

顺带澄清一次误报：另一条 FAIL「作答窗接近一整天」是**我的探针写错了** ——
把 pg 返回的 interval 对象当字符串解析。实测 86280 秒 = **23.97 小时**，
数据本身是对的。

##### 修好了，但**没有重跑**

  · 所有历史时间戳改用 `dayBefore(todayIso, N, 'HH:MM:SS')` ——
    **钉在那个过去日历日的钟点上**，跟几点跑脚本无关；
  · 回读校验加了三条当天断言（复习流水 / 错题重练 / `lastReview`），
    夹具再也没法把这种东西发出去；
  · 前置检查里「当天的用户活动」**排除夹具自己的行**（`id NOT LIKE 's12f_%'`）
    —— 否则夹具一旦自己写错时间，它就再也修不好自己。守护用户证据的是
    那道排除外来行的闸，不是这一条。

合同的 STOP 条件写得很明确：「夹具的真库结果没通过任何一条冻结分布」→
**停下、不即兴删数据、如实报告持久增量**。所以到此为止，
**没有第二次写入**。

##### 当前 staging 的持久增量（如实记账）

  · `DictEntry` **+51**（8 → 59），原有八行未动；
  · `s12f_acceptance_student` 及其全部 `s12f_` 资源**已存在**，
    其中 **14 条复习流水的时间戳落在今天** —— 这是那个缺陷的痕迹；
  · t1–t8 **逐字节未变**；`AuditLog` 未变；通知 0 / 0；考勤 0。
  · 全局计数的变化正好是本账号自己的行：申诉 2 → 4、错题 3 → 23。

> **一处与冻结事实的出入，与我无关但要记下来**：v1.0 收尾时记录的是
> 申诉 1 / 审计 1，本轮基线读到的是 **2 / 2**。多出来的那条申诉建于
> `2026-08-30T06:06:44Z`，挂在 t6 的答卷上，动作是
> `morning_quiz.appeal.create` —— **是有人在 staging 的学生端上手动提的**，
> 不是我写的（v1.0 全程零写入，快照前后逐字节相同）。说明有外部操作者在用
> 这个库，隔离结论要按时间戳读。
...
| 命令 | exit | 结果 |
| --- | --- | --- |
| 词典 spec（RED → GREEN） | 0 | RED `38 failed \| 10 passed (48)` → **48 passed** |
| 夹具 spec | 0 | **79 passed**（新增 7 条时间戳归属回归） |
| `node --check` ×2 | 0 | 干净 |
| api 全量 | 0 | 96 files / **1477** passed |
| api `tsc` / `nest build` | 0 / 0 | 干净 |
| student-web 全量 + tsc + build | 0 | 26 / **919** |
| web 全量 + tsc | 0 | 37 / **247** |
| `git diff --check` | 0 | 干净 |

那 10 项在 RED 里「通过」的是**假绿** —— `expect(...).toThrow()` 形式的
否定断言，模块缺失时因为取模块自己抛而通过，**不计入 RED 证据**。

##### 还差什么

**授权重跑一次夹具**。重跑会先删掉自己拥有的 `s12f_` 行再重建
（前置检查已能区分「夹具自己的当天行」与「用户造的当天行」），
然后当天就干净了。重跑会**重新生成 PIN**，届时单独交接。
在那之前，人工走查清单**一步都不要开始**。

#### 返工 2/2 —— 重建一次，当天干净了（2026-08-30）

`contract_version: 1.2` · 基线 `47da846`。**没有任何代码改动** ——
时间戳修复在返工 1/2 就已提交（`dc13cf2`），这一轮只是拿它重跑一次，
外加一次普通登录探针。词典**一行没动**，也没有再跑补种脚本。

##### 重跑之前的安全证据（只读）

  · 词典总数 **59**，51 行清单逐字节相同，原有八词哈希
    `9abe75b4c80501317644a391f2cf282c` 未变，没有第 60 行；
  · 账号在，**`lastLogin` 仍是 null**、`pinFailedCount` 0、`authVersion` 0
    —— 没有任何人（无论普通登录还是免密）登过它；
  · 当天只有那 **14 条复习流水**，而且 **14 条全部带 `s12f_` 前缀**
    （夹具自己造的），其余全为 0：任务行 / 答卷 / 正式测试 / 考勤 /
    错题重练 / `lastReview` / 申诉 / 浏览记录；
  · 十六张表逐一扫过，**没有一行不带前缀**的 S12F 数据 ——
    也就是说，那个缺陷的痕迹**完全落在夹具自己拥有的行里**，
    没有一丝人类活动混在里面。

##### 重跑

夹具跑了一次，退出码 0，回执写着「之前是否已存在：**是（已按 `s12f_`
前缀重建）**」。写入范围仍然只有它自己拥有的那些行。

##### 44 项真库核对**全过**

九条强制的「当天归零」断言，全部按 **SGT 零点**（`2026-08-29T16:00:00Z`）算：

| 断言 | 实测 |
| --- | --- |
| 当天 `WordReviewLog` | **0**（返工前是 14） |
| 当天 `StudentWord.lastReview` | **0** |
| 当天错题 `lastPracticedAt` | **0** |
| 当天 `DailyLessonCompletion` | **0** |
| 当天答卷 | **0** |
| 当天正式测试 | **0** |
| 考勤 | **0** |
| 当天申诉 / 浏览记录 | **0 / 0** |
...
作答窗**按秒数值测量**：`86280 s = 23.97 h`
（返工 1/2 报的那条 FAIL 是我探针把 pg 的 interval 当字符串解析，
数据本来就是对的）。

冻结分布逐条复核：阅读 12 份（判完 10 · 待判 2 · 12 个不同篇目 ·
12 个不同的过去日历日 · 分数 0/4/4/5/5/5/5/5/6/8）、逐题 72 条且
**逐题得分之和 == 总分**、历史任务行 13（完成 9）、正式测试 11（含 0 分与
满分，四种题型齐）、生词 50（新 10 / 学 12 / 复 20 / 掌握 8，重学 5，
到期 21 / 将来 29，**50 个全部有词典音标与释义**）、复习流水 198 跨 14 个
历史日、错题 20（未销账 16 / 已销账 4，6 种题型，三种原因齐）、
合成申诉 2 条（正文与批注都带 `STAGING SYNTHETIC`）、今天的卷子
`ielts_reading` + `passage_pick` + 10 题 + 4 种任务类型、
`blossom` 在词典与原文里且**不在生词本、不是任何答案**、
第 4 题是无选项的 `sentence_completion`。

##### 隔离

t1–t8 的完整指纹**逐字节相同**（哈希 `0938a11c91a0bc2dac037cb1d6c51c55`
前后一致），t6 的生词 / 流水 / 错题 / 答卷四组逐项相同。
词典 59 行未变。全局计数前后完全一致：申诉 4 / 审计 2 / 通知 0-0 /
考勤 0 / 错题 23 —— 重建把自己的行删了又建回同样的数目。
学生集合仍是八个夹具 + 本账号。**核对窗口内没有观察到外部操作者的改动。**

##### 普通登录探针（三个请求，全是读，零业务写）

```
POST /api/student-auth/login   -> 201   body keys ["name","pin"]
GET  /api/student-auth/me      -> 200
GET  /api/lesson/today         -> 200
```

  · **走的是普通姓名 + 密码那条路**，没有碰免密端点，也没有点那个
    「一键登录测试六号」按钮；
  · 登录响应认出 `s12f_acceptance_student`，`needDisambiguation` 为 false；
  · `/me` 同样认出它；
  · `/lesson/today` 回 **`nextAction.kind = ready_to_start`**，
    三段全是 `todo`（阅读 10 题 / 背词 0-21 / 补段 0-5），
    `targetsFrozenAt` 是 **null** —— 任务行还没被创建，正是干净的定义；
  · **没有调用 `POST /lesson/start`**，也没有任何别的写。

登录页实测长这样：**「姓名」「密码」两个输入框 + 登录按钮**，
下面另有那个 staging 免密按钮 —— 是两个分开的控件，用户不会误点。

> **一处如实说明**：「退出登录并确认令牌被清掉」这一步**没有在浏览器里
> 走完**。要在真页面上登录就得把 PIN 打进输入框，要注入会话就得把令牌
> 贴进脚本 —— 两者都是合同禁止打印的值。所以退出是在传输层完成的：
> 令牌只存在于探针进程与一个仓库外的临时文件里，**用完即删**；
> 并且实测确认学生端 `/login` 这个源上的 `localStorage` 是**空的**。
> 页面上那个「退出登录」按钮本身的行为**这一轮没有验证**
> —— 它属于用户走查的第 23 步。

##### 一处真实度缺口（不属于任何冻结验收项）

主页的**课程连续天数显示 0**，不是 13：`lessonStreak` 只认
`readSource ∈ {student, teacher}` 的日子，而历史任务行写的是 `lesson`。
生词本那条连续天数是对的（**14 天**，来自 198 条复习流水）。

这不在任何一条冻结 AC 里，也不影响那 23 步中的任何一步。合同 v1.2 明确
写着「超出已提交的时间戳修复之外的代码改动一律 NEEDS_HUMAN」——
所以**不改代码**，只把它写进走查清单的「已知」栏，免得用户把它当缺陷记。

##### 当前 staging 的持久状态

  · `DictEntry` **59**（8 原有 + 51 清单），一行未改；
  · `s12f_acceptance_student` 与它的全部 `s12f_` 资源，**当天干净**；
  · 一次成功的普通登录（`lastLogin` 被服务端更新 —— 这是登录本身的
    正常副作用，也是唯一一处）；
  · t1–t8 与其余全局数据**逐字节未变**。
...
#### 12I —— 界面那一半（2026-08-31）

`task_id: S12I-STUDENT-WEB-PRESENTATION-LOCAL` · contract v1.0 ·
base `134ea0c` · 实现提交 `019330c`。
**纯本地：没有部署、没有 staging、没有数据库、没有重建验收账号、
没有动 API / Prisma / 夹具 / 依赖 / Railway。**

S12H 把服务端那一半做完了但**界面一个现象都还没消失**。这一轮把九个
用户看得见的缺陷逐个改掉。

##### 量出来的 RED（对着 `134ea0c`）

```
npx vitest run src/__tests__/s12i-presentation.test.tsx
→ exit 1 ·  25 failed | 3 passed (28)   套件正常收集并执行
```
...
| 现象 | 失败断言 |
| --- | --- |
| 客观题显示「还在判分」 | 客观题被整卷 pending 盖住了：expected `'pending'` to be `'correct'` |
| 逐题分数被整卷旗子藏起来 | 找不到 `marks-pq-1` |
| 「正确答案 / 参考答案」两行重复 | 同一句话挂了两个名字：expected 0 to be 1 |
| 历史详情没有原文 | 找不到 `passage-toggle` |
| 写死的「四道题都答完了」 | 十题的卷子里 expected …to contain `'10 道题都答完了'` |
| 没教过的词直接考 | 找不到 `teaching-card` |
| 内部枚举串上屏 | 原始枚举 `multiple_choice` 打到界面上了 |
| 错题本一屏堆满 / 标题重复 | 没有按卷子分组：expected 0 to be 2；评语默认就摊开了 |
| 重练进度含糊、原文无定位 | 找不到 `round-progress` / `locate-toggle` |
| `drill` 落不了地 | `Cannot read properties of undefined (reading 'kind')` |
| 没做完也能看总结 | expected `'/lesson/summary'` to be `'/today'` |

那 3 项在 RED 里通过的不算证据（本来就成立的否定断言）。

##### 消费的服务端字段

三个都来自 S12H，**一个都不在浏览器里重算**：

  · `gradingStatus`（`auto_graded` / `marked` / `pending_marking` /
    `not_answered`）→ 决定逐题的「答对 / 答错 / 还在判分 / 没有作答」；
  · `answerDisplay`（`primaryKind` / `primaryValue` / 可选 `rubricValue`）
    → **答案展示的唯一权威**。措辞（「正确答案」「参考答案」「评分要点」）
    留在客户端，API 里一个中文标签都没有；
  · `gradingSummary` → **四件事分开说**：「已自动判分 N 题」、
    「老师已批改 M 题」、「K 题等老师批改」、「J 题没作答」。
    计数为 0 的整段不出现，分隔符按下标加，开头结尾都不挂点。

旧服务端不发这三样时**行为一字不改**：`questionOutcome` 保留原来的推断，
`answerRowsOf` 用 `correctAnswer` / `referenceAnswer` 自己拼一份 ——
**并且同样用归一化去重**（NFKC → 去首尾 → 折叠空白 → casefold，
只比较、不改展示值）。

> 一处顺带的诚实修正：**没判的题现在整行不出现**，而不是显示「— / 2 分」。
> 破折号看着像「判了但没给分」；不出现才是「还没判」。原来那条
> 「等人工判分的题不显示 0 分」的断言依旧成立，只是判据从「显示破折号」
> 变成「整行不在」。

##### 原文回看

数据一直都在 —— `redactSnapshotForStudent` 的白名单里就有 `passage` 与
`passageTitle`，`history-detail` 与交卷结果共用同一个投影。**只是从来
没有渲染过。**

放在 `ResultView` 里，所以交卷结果页与历史成绩详情页**同时**有了：
默认收起、`查看原文` 展开、能再收回、**整份只渲染一次**（`passage_pick`
的卷子把同一段原文存在每一道题的快照里，取第一份就够）、纯 React 文本、
自己的滚动容器 + `max-h-[60vh]` + `break-words`。
**载荷里没有原文就不显示控件**，绝不造一段。

##### 先教后考

`/vocab/due` 早就带 `needsFirstTeaching`，这一屏以前没用它。现在：

  · **稳定分区**：教学卡在前、复习卡在后，**组内保持服务端顺序**，
    不过滤、不去重、不重排组内顺序；
  · 教学卡摊开词、音标、释义、词性、来源、完整例句 ——
    **只在词形是例句的精确子串时才高亮**，对不上就原句照旧；
  · 「我看过了，开始记忆」**不发任何请求**，只把同一个词切到回忆模式；
    真正写库的仍然只有随后那一次正常评分；
  · 已有的写入闭锁、重试、`requestId` 复用、防连点、跳过、撤销、
    失败不翻页 —— 一条都没动。

##### 错题本

  · 题型标签表从 8 个补到 18 个（含 `multiple_choice` / `matching_features`
    / `classification` / `multi_match` / 四种 completion 等），
    **兜底从「原样显示」改成「其他题型」** —— 内部标识符再也上不了屏；
  · 按**卷子 + 日期**全局分组（不是只合并相邻的），组头出一次标题与
    日期，卡片里不再重复；组的顺序按**首次出现**，组内保持服务端
    顺序，**不重排、不过滤、不去重**；标题相同但不同天的绝不合并；
  · 默认卡片只回答「哪一题、错在哪」：题号型别、状态（未掌握 / 重练中 ·
    连对 N / 已掌握）、题干、你的答案 vs 正确答案；
  · 分数、收录原因、要点、范文、老师评语、解析、原文依据全部收进
    **「展开详情」**；「看那次的整份成绩」的入口保留。

##### 重练的原文定位

  · `本轮第 1 / 10 题` 与 `错题本仍有 16 题` **分开说**（旧的两个 testid
    保留成屏读专用，既有测试不失效）；
  · `查看原文并定位` 默认收起；
  · 证据句**是原文的精确子串**才高亮那一处；
  · **没有证据句、或者对不上** → 给完整原文 +
    「这道题没有存下证据句的位置，下面是完整原文。」
    **绝不模糊匹配猜一个位置** —— 学生会以为那就是出处。
    （全仓库没有一处写 `answerContent.evidence`，所以现阶段几乎总是走这一支，
    这正是它该有的样子。）
  · 作答 / 翻卡之前，答案材料一个字都不进 DOM —— 原样保留。

##### drill 与总结页守卫

  · `NEXT_ACTION_KINDS` 10 → **11**，`drill` 映射到**已有的**
    `/mistakes/practice`；不新开页、不新开端点、**不看后端的 `href`**；
  · G9 穷尽守卫跟着从 10 改到 11，可跳转的 kind 从五个变六个 ——
    是**跟着后端联合走**，不是把守卫放松；
  · 总结页现在要求 `kind === 'summary'` **且** `allDone === true`
    **且** `completed === total`。判断在 `setPhase({s:'ready'})` **之前**，
    所以不合格时总结的 DOM **一帧都不会出现**。

##### GREEN

| 命令 | exit | 结果 |
| --- | --- | --- |
| 聚焦 spec `s12i-presentation` | 0 | **28 passed**（RED 时 25 failed / 3 passed） |
| student-web 全量 | 0 | 27 files / **947** passed（26 / 919 → +1 file / +28） |
| student-web `tsc --noEmit` / `vite build` | 0 / 0 | 干净 |
| api 全量 + `tsc` | 0 | 100 files / **1545**（一行没改） |
| web 全量 + `tsc` | 0 | 37 files / **247** |
| `git diff --check` · 工作区 | 0 · 干净 | |

**八份既有测试跟着新口径改了**，全部是「测的是旧写法」而不是放松断言：
`answer-row-*` 的 testid 改名、没判的题整行不出现、错题本的标题挪到组头、
稠密材料要先展开、原文进了定位区、非 summary 的 kind 从 9 个变 10 个、
以及两处总结页夹具补上 `allDone` 与 `completed === total`。

##### 请求边界（逐条核对）

**没有新端点、没有新路由**（`drill` 用的是已注册的 `/mistakes/practice`）、
**不消费后端 `href`**、没有旧端路径、没有 `mq:*` 键、
URL / 查询串 / 请求体 / 存储里**没有任何身份字段**、
没有多出一次业务写（「我看过了」是纯本地状态）、
归属失败的处理与既有的课程 / 正式测试 / 历史 / 错题 / 结果请求边界
一字未动。

##### 返工 1/2 —— 两个阻断项（2026-08-31）

基线 `da6ab12` · 修复提交 `d9cc12c`。复审拉出两条，两条都是
**我的第一版自己的测试照不出来的**。

**B-1 把人判的说成了机器判的。** 第一版把 `autoGraded + marked`
加起来渲染成「已自动判分 N 题」。只要 `marked > 0`，那句话就是假的 ——
老师亲手批的题被当成了自动判分。而我那条用例恰好用了 `marked: 0`，
所以它从来没照到这个缺陷。现在四个计数**各说各的**，
分隔符按下标加，顺手修掉了另一个：只有「没作答」时，摘要以一个
孤零零的「·」开头。

**B-2 分组其实没做成。** 第一版只合并**相邻**的同组。可服务端是
按天倒序、同天按收录原因排的，**不是按卷子排** —— 同一份卷子的两条
错题中间完全可能夹着别的卷子，于是标题照样重复。现在用 Map 做
**全局分组**：组序按首次出现、组内保持服务端顺序、不过滤不去重、
同名不同天绝不合并（`quizDay` 就在键里）。

量出来的 RED（对着 `da6ab12`）：`7 failed | 31 passed (38)`。
关键失败值：`expected '已自动判分 2 题' not to contain '自动判分'`、
`expected '已自动判分5题·…' to contain '已自动判分3题'`、
`摘要以分隔符开头："· 2 题没作答"`、
以及交错夹具（A/day1、B/day1、A/day1、A/day2）上的
`expected 4 to be 3` 与「Paper A / 8-25 的标题重复了」。

GREEN：聚焦 spec **38 passed**；student-web 全量 27 files / **957**；
tsc / build、api 100 / 1545、web 37 / 247 全部 exit 0。
**没有删掉或放松任何既有断言。**

> 另更正一处我上一轮报告里的错话：我把「测试放在
> `src/__tests__/`」写成了范围偏离。**它不是** —— 合同本来就允许
> 直接相关的测试放在那个目录。那一行自我报告是多余的。

##### 还差什么

界面这一半做完了，但**用户还没有再走一遍**。要重新验收，还需要：
夹具那一半（真实原文的历史卷子、留一批没教过的词）、
一次部署、以及用户本人重跑那 23 步。**阶段 12 仍然是
`USER_ACCEPTANCE_FAILED / REDESIGN_IN_PROGRESS`。**

---

#### 12J —— 夹具那一半，与账号重建（`task_id: S12J-FIXTURE-REALISM-AND-LIVE-HANDOFF`）

`contract v1.0` · `rework 0/2` · 基线 `9f3a3c7` → `fbacb44` / `<S12J-2>`。

12H 修好了服务端，12I 修好了界面，**可账号本身还是假的** —— 用户走查时
看到的那些「原文是一句话」「每天的正确答案一模一样」「一上来全是复习卡」
「连续天数 0」，一条都不在代码里，全在夹具里。这一轮把夹具补齐，
并且**把账号真的重建了一次**。
...
| 现象 | 根因 | 现在 |
| --- | --- | --- |
| 「查看原文」点开是一句话 | 十二份历史卷共用一句占位原文 `【S12F 合成阅读 · …】…` | `s12f-reading-content.js`：十二篇原创四段说明文，每篇 1400–1750 字符 |
| 十几条错题的「正确答案」一模一样 | `histAnswerTexts(taskType)` 是**按题型**查的硬表 —— 不管哪一天、哪一篇，填空题的答案都是 `rainwater` | 每道题带自己的 `answerText` / `partialText` / `wrongText`；抽查 18 道题得到 18 个不同题干、17 个不同参考答案 |
| 错题重练**永远**说「没有存下定位」 | 全库没有一处写过 `answerContent.evidence`（12G 查过：有读没写） | 82 道题里 80 道存了证据句，且每一句都是**本篇原文的逐字子串** |
| 词汇一上来全是复习卡，教学卡验不到 | 到期队列里只有 2 个没教过的词，且排在后面 | 六个新词的 `due` 最靠前（`-30 - i` 小时），教学卡必然先出 |
| 主页连续天数恒为 0 | 历史任务行写 `readSource: 'lesson'`，而连胜只认 `student` / `teacher` | 改成 `student`；线上实测 `streakDays = 9` |

顺手改掉的两处内部矛盾（同属「答案与题目要对得上」）：错题的 `taskType`
以前按序号轮换，会把一条填空题标成「选择题」，现在跟着它挂的那道题走；
今天那份卷子有两道题与原文相反（花园是**地理老师**提的，不是学生；
「市政厅出了钱」在原文里是 FALSE 而不是 NOT GIVEN），改的是**题干**
不是答案键 —— 改键会让头三道 TFNG 全变成 FALSE。

**故意留了一个空**：第十二篇的第 6 题没有证据句，而它正好会进错题本。
客户端那句「这道题没有存下证据句的位置」的诚实兜底，得留一条能验到。

##### 账号卡死了，得先能重建

用户那次失败的走查在库里留下 **32 行不带 `s12f_` 前缀**的业务数据
（任务行 1 / 答卷 1 / 词汇测试 1 / 逐题答案 10 / 复习流水 19）。
S12F 的重跑闸门看到这些行**无条件拒绝** —— 立意是对的（那是用户的验收
证据，重建会毁掉它），但后果是账号**既修不好、也不能重来**。

改法不是把闸门拿掉，是**给它一个前置**：

```
导出整个账号（写盘 → 读回 → 哈希对得上）→ 才允许按学生 id 删
```

- `exportScopes()` 列出 16 张表；`redactRow()` 把 `password|pin|token|secret|salt`
  形状的字段一律换成 `[redacted]`（实测：导出文件里搜不到任何 bcrypt 串）；
- 导出写完立刻读回来重算 SHA-256，并核对行数；对不上就抛，整个事务回滚，
  **一行都不删**；
- `assertRerunSafe` 现在要求 `verified === true`、64 位哈希、
  且导出的账号行数 ≥ 要删的行数；缺一样就继续拒绝；
- `S12F_EXPORT_DIR` 成为第九道环境闸门。

`wipeStatements({ includeStray })` 是**纯函数**，所以杀伤半径可以脱库钉死：
每一条 DELETE 都限定在本账号 id 或 `s12f_` 前缀上、t1–t8 的 id 一个都不出现、
全部带 WHERE、全部是 DELETE、子表排在父表前面、`User` 行不在删除范围内
（账号是 upsert 的）。另有一条：**凡是重建会删的表，都必须先在导出范围里**。

> **踩了一次**：`S12F_EXPORT_DIR` 加进了闸门却忘了加进 `ENV_AT_STARTUP`
> 那张白名单，于是明明传了值，脚本还是报「没给」。现在快照由
> `ENV_SNAPSHOT_KEYS` 生成，并有一条 spec 拿 `assertEnvGates` 的源码
> 反查它 —— 下次漏写当场就红。
...
| 服务 | 部署 id | 状态 |
| --- | --- | --- |
| `stg-student-web-spike` | `c8bf2385-db37-405f-85ec-9e4afaa99e19` | SUCCESS |
| `stg-api` | `d2e81a4e-eafe-4197-a176-983cff3ead31` | SUCCESS |

回滚锚点仍是上一版：spike `5323fc0e-…`、api `6f08ca63-…`。
`/api/health` 200（`uptimeSec=74`，新进程）、`/api/health/ready` 200 `db:"up"`。
线上 JS 里 `已自动判分` / `老师已批改` / `等老师批改` / `查看原文` /
`我看过了` / `这道题没有存下证据句的位置` 各出现一次 —— S12I 的确在线上。

重建（一个事务）：删掉 **22** 行（这是闸门自己的计数口径，它不数
`AnswerScript`；连同按 `submissionId` 级联删掉的 10 行，实际清掉的
无前缀行是 32 行，与前置快照吻合）。导出文件 402 536 字节 /
614 行（账号 411 · 共享 203），SHA-256
`e92762d5d92546bfd704786b825785b158a45e396207771f8ad319e5fdbee945`。

前后快照对比：t1–t8 的指纹哈希 `8d4403844f9ece6ef96a72393ab6fa3e`
**逐字不变**，词典 59 行不变，全局计数不变；账号名下的无前缀行
全部归零；`placeholder_questions` 72 → 0、`with_evidence` 0 → 80、
`untaught_due` 0 → 6、`student_source_days` 1 → 12。

##### 只读冒烟（合同禁止的动作一个都没做）

没点「开始今天的课程」、没评词、没答错题、没开正式测试、没提申诉。
除登录本身（更新 `lastLogin`）之外全是 GET。

- 普通姓名 + PIN 登录 201，认出 `s12f_acceptance_student`；`/me` 200；
- `/lesson/today` 200：`stage=reading`、`nextAction=ready_to_start`、
  三段全 `todo`、`completed 0/3`、**`streakDays = 9`**（改之前是 0）；
- `/vocab/stats`：50 词 · 到期 21 · 累计复习 198 · 今日复习 0；
- `/vocab/mistakes`：20 条 · 12 个不同篇目 · 20 个不同题干 · 19 个不同参考答案；
- `/vocab/mistakes/practice-queue`：10 条，**10 条都带完整原文与证据句，
  10 条都能在原文里逐字定位**；
- `/morning-quiz/history-by-name`：12 份 · 篇目名 12 个各不相同 · 已判 10 · 待判 2；
- 抽三份详情：`gradingStatus` 出现 `auto_graded` / `marked` / `pending_marking`
  三种；待判那份 `gradingSummary = {auto 3, marked 0, pending 3}`、
  `totalScore = null`、`scoresPending = true`；**`markedById` 不在响应里**；
  `answerDisplay` 只出一行；每题原文 1417–1655 字符；
- 学生端 `/login` `/today` `/scores` `/mistakes` `/vocab` `/account`
  全部 200 且带 `X-Student-App: v2`。

冒烟之后再拍一次快照：当天计数仍然全是 0，t1–t8 / 词典 / 全局 / 账号行数
全部与重建后一致。**今天那一课的第一个动作仍然属于用户。**

> **限速是真的**：`/morning-quiz/history-detail` 连打会 429（`retryAfter` 约 20 秒）。
> 第一次抽样时我没留意，读到的空数组是被限速挡回来的、不是数据缺失；
> 上面的数字是加了 25 秒间隔重跑出来的。

##### 走查清单重写

23 步不变，但顺序按线性流程重排 —— **补段（错题重练）排在今日总结之前**，
这一步以前根本不在清单里。删掉了「连续天数会显示 0」那条免责说明
（缺陷已修，不该再当成已知），删掉了写死的题数，加上了教学卡、
逐题判分状态、答案只出一行、查看原文、错题按篇目分组这几条新的判据，
并在开头写明**上一次那趟走查不计成绩，请从第 1 步重新开始**。

##### 还差什么

**用户本人还没走。** 阶段 12 是 `AWAITING_USER_ACCEPTANCE`，
不是 PASS。另外两件没验到的事照实记：

- **教学卡先出，只在库里验到，没在线上验到** —— `/vocab/lesson-cards`
  在课程开始之前返回 0 张卡（`lessonContext: false`），而「开始今天的课程」
  是合同明令禁止我点的。库里的证据是 `untaught_due = 6` 加上夹具计划里
  「按 `due` 升序的前六张全是没教过的词」这条断言；
- **触屏点词手势**仍然只有真设备能回答（自 12E 起未变）。

---

#### 12K —— 成熟度重建计划（`task_id: S12K-STUDENT-APP-MATURITY-REBUILD-PLAN`）

只勘察、只出计划，一个字的代码都没改。产出是一份十四节的重建计划：
现状证据（源码 file:line + 只读 staging 实测）、每条投诉的根因、目标信息
架构、十屏的手机 / iPad 线框、后端与前端的具体改动、五阶段十四个可独立
评审的任务、每个任务的 RED/GREEN、响应式验收矩阵、无障碍验收标准、
生产词典发布闸门、部署与回滚、全新账号验收流程、风险与未验证项。

**它没有作废，只是被试点插队了。** S12L 之后如果决定继续做完整重建，
从这份计划的阶段 M（生词学习中心）接着走。

---

#### 12L —— 试点最小改动并上线（`task_id: S12L-STUDENT-PILOT-MVP-IMPLEMENT-AND-LIVE`）

`base d7795d5` → `0887733`（API）→ `92fbbb8`（学生端）→ `<docs>`。

**策略变更**：用户把目标从「先把架构做对」换成「**先让 5–10 个真学生
用起来，按他们撞到的东西修**」。所以这一轮只做那些真学生第一天就会
撞到的东西，其余一律不碰。
...
| 现象 | 根因（改之前） | 现在 |
| --- | --- | --- |
| 错题本半成品挡着今天 | 补段是三段之一，`lessonProgress` 分母恒 3、`deriveStage` 与连续天数都等它 | 整个功能**暂停**：`pilot-flags.ts` 一个常量；分母变 2；服务端一次错题查询都不发；四个端点 503 `feature_paused`；**采集侧照常**，学生的 20 条错题一行没删 |
| 三张卡点了没反应 | `Today.tsx` 有意把三段做成纯状态摘要 | 卡片可点（**只导航、不推进**），不可点的说明为什么；主行动仍然只有服务端那一个 |
| 学了 21 个词只考 10 题 | `quiz-eligibility.ts` 的 `taught.slice(0, MAX_QUIZ_ITEMS)`，界面上一个字都没提 | 上限取消，**一词一题**；下限 4 → 1（「今天只学了两个词」是正常的一天）；队列本身改为封顶 30；凑不齐题就明说，不发半份卷子 |
| 二十一题里只有一道拼写 | `formalTypePlan` 是「第一个能拼的出拼写、其余选择题交替」 | 四种题型**按位置轮转**；四个词仍是四种各一道，21 个词是四种各五六道 |
| 拼写题只给一句挖空的英文 | 作答前把 `translation` / `phonetic` / `headword` 全遮了（它们确实是答案） | 新增 `cue`（词性 / 中文释义 / 英文释义）**作答前就下发**；选择题恒为 null；`cueFor` 把混进答案的那一项判掉 |
| 课程里教过的词被翻成挖空复习卡 | 队列是「今天到期的全部词」，`needsFirstTeaching:false` 的发复习卡 | 课程内**每一张都是教学卡**；主动回忆搬去自由复习（那边的评分 / 撤销 / 弱网补传一条没删）；词段进度改数「队列里教过几个」 |
| 生词本 50 个词铺到底 | `/vocab` 无搜索、无筛选、无分页 | 加了按词头搜 + 四档筛选（全部 / 学习中 / 待复习 / 已掌握）+ 空态。**不做**服务端分页与收藏 —— 记为试点后的事 |
| 自测直接开考 | 题量由服务端拍板 | 先一屏设置（5 / 10 / 20 / 全部，封顶 30），并**明说刷新会重开一份**；正式测试仍然断点续答 |
| iPad / 电脑只用中间一条 | 全站唯一外壳 `max-w-md`（448px），1366 上左右各空 459px | 按断点放宽；375 一个字没变；返回挪到顶部；结果页 ≥1024 左原文 / 右题目 |
| `marked` 与 ISO 时间戳上屏 | `ResultView` 直出服务端值 | `lib/format.ts`：`已批改` / `8月26日 08:51`（新加坡时间） |

##### 测出来的 RED

服务端 `src/lesson/s12l-pilot.spec.ts`：**6 失败 / 5 通过**（分母 3、
`lessonComplete` 卡在补段、`deriveStage` 停在 `vocab_test`、21 个词只挑出
10 个、教 1 个词 `insufficient_items`、拼写只出现 1 次）。

学生端 `src/__tests__/s12l-pilot.test.tsx`：**18 失败 / 0 通过**
（三张卡都不是按钮、错题页在发请求、教过的词是复习卡、没有 `question-cue`
与 `quiz-intro`、生词本没有搜索框、自测没有设置屏、正文容器没有响应式
断点、结果页没有 `result-split`、`marked` 与 ISO 串照旧上屏）。

##### 部署与线上实证

先学生端后 API（**顺序是必须的**：新前端不再打错题端点，所以旧 API 也
兼容；反过来会让旧前端撞上 503）。

| 服务 | 部署 id | 状态 | 回滚锚点 |
| --- | --- | --- | --- |
| `stg-student-web-spike` | `18c6b2a6-caff-4788-8ba8-189773244d3f` | SUCCESS | `c8bf2385-…` |
| `stg-api` | `c097b507-c2c2-4b36-9a58-d9fce15bae61` | SUCCESS | `d2e81a4e-…` |

`stg-web` 与 `Postgres` **未部署**（这一轮没碰 `apps/web`；教师看板那处
`segments[2]` 的下标改成按 `key` 找，改在 `apps/api` 里）。

线上只读实证（验收账号，除登录外零写）：

- `/today`：**今天完成 2 / 2**、错题卡「暂未开放 · 不计入今日完成」、
  主行动是「看今天的总结」——改之前它会是「开始错题重练」（补段 0/5）；
- 三张卡：阅读与单词是 `<button>`，错题是 `aria-disabled` 的 `<li>`；
  点阅读卡 → `/lesson/reading/result`；
- 结果页：`result-split` 在 1366 下是 `grid 523px | 733px`，原文在题目
  左边，正文列 1280px（改之前 672px）；状态「已批改」、时间「8月26日 08:51」；
- 375px：`/today` `/vocab` `/scores` `/mistakes` `/account` `/vocab/selftest`
  与「结果页 + 展开原文」全部 `scrollWidth == clientWidth == 375`；
- 生词本：搜 `achi` → 1 条；「已掌握」→ 10 条；「待复习」→ 6 条；
  搜 `zzzz` → 空态文案；
- 自测：设置屏 5/10/20/全部（30），选 5 → 只发一个
  `GET /vocab/quiz?limit=5`，**零写**；
- `GET /vocab/quiz?limit=20`：20 道题**全部带 cue**，选择题 0 道带 cue，
  **0 处把 headword / answer 漏进 cue**；
- `GET /vocab/mistakes` → **503 `feature_paused`**；
- 退出登录：令牌与 localStorage / sessionStorage 全清，**零请求**（纯本地）。

前后数据库对比：t1–t8 指纹、词典、全局计数、当天计数、`s12f_` 行数、
账号行**全部逐字不变**；唯一差异是验收账号的一次 `lastLogin`。

##### 两件没能在线上验到（照实记）

`21 题` 与 `课程内全是教学卡` **这一轮没能在线上验**：用户在本次部署
**之前**已经走完了今天那一课。今天的任务行是旧规则冻结的
（队列 21、`vocabCursor 21`、`stage vocab_test`），而当天的正式测试
**已经交卷**（`total: 10`、`items: 10`、`spelling: 1`、`cue: 0`）——
那份 attempt 一旦提交就是不可变的成绩，`start` 只会把它原样返回。

那份线上数据本身正是这两个缺陷的证据：**教了 21 个词、卷子只有 10 道、
只有一道拼写、一条线索都没有**。修好之后的行为由 RED→GREEN 的单元与
集成测试覆盖（`selectEligible` 21→21、`formalTypePlan` 四种各 ≥5、
attempt 的题数与词数必须相等、`cueFor` 的七条契约）。

**下一个新账号的第一天就会在线上产生这两条证据** —— 试点第一天即可确认。

---

#### 阶段 12 的状态

**`PILOT_RUNNING`** —— 不是 PASS，也不再只是「等那 23 步」。

试点最小改动已经部署到 staging，流程见
[docs/pilot/s12l-pilot-plan.md](../pilot/s12l-pilot-plan.md)：
5–10 个学生、一人一个账号、三天、五个反馈问题、六条立刻停止的红线。

**名单由用户定**；跑起来也不等于阶段 12 通过 —— 那仍然要走冻结的人工
验收。**阶段 13 一行未开始。**
...
上面那句「下一个新账号的第一天就会在线上产生这两条证据」，S12M 当天就
兑现了：内部冒烟账号第一天拿到 **21 张教学卡 → 21 道题**，其中 11 道带
线索，`unansweredLeakingAnswer: 0`。S12L 修的两个缺陷，在真数据上确认
修好了。

**当时写了什么**（`apps/api/scripts/pilot/content/`，三档各两天）：
...
六篇**原创**文章，每篇 10 题（6 道服务端当场判 + 4 道老师批）、21 个
目标词；三档两天合计 60 题、124 个不重复的词。版权上干净 —— 不存往届
真题原文。

**发布方式**：`scripts/pilot/prepare-pilot-week.js --day=<日期>`，
每天一跑，**幂等**，无直接 SQL。它只写 `p1_` 前缀下的东西，结束前逐条
断言答卷指纹 / 非试点生词 / 复习流水 / 错题申诉 / 当日任务行未变，
一样不对就回滚。词典只补录当周用到的词，且**只在不存在时插入** ——
唯一的例外是它自己插过的（带 `pilot_w1`）那些行，用来纠自己的错别字；
staging 原有的 59 条一条都碰不到。

**词汇结转**由脚本顺手排期：当天 21 个词排到今天到期，**前一天那批还没
复习过的**推到 2026-09-14。否则周二的队列会是 42 个词。已经复习过的词
不动 —— 那归 FSRS。周二发布当场就验证了：队列是 **21**，不是 42。

**开跑手册**见 [docs/pilot/s12m-launch.md](../pilot/s12m-launch.md)，
答案与批改标准见 [docs/pilot/s12m-teacher-pack.md](../pilot/s12m-teacher-pack.md)
（那份**不发给学生**）。

**这段是 S12M 当时的范围，已经被 2026-09-01 的 S12P 补齐**：五档、
周一至周五的完整现状与线上证据见下方 S12P，不再把「三档两天」当成
当前上线范围。

**仍然不是 PASS。** 试点跑起来只是把状态从 `PILOT_READY` 推到
`PILOT_RUNNING`；阶段 12 通过仍要用户本人走完冻结的 23 步。
**阶段 13 一行未开始。**
...
真人进来之前的最后一道坎：老的 `register` 是**认领**，前提是教师先在
花名册上建好那一行、还替他设好难度。`student_not_found` 就是这个前提的
回声。请十个真人试用，这个前提站不住 —— 而让教师替一个十五岁的人猜他
该上哪一层，猜得比他自己差。
...
| 端点 | 干什么 |
| --- | --- |
| `POST /student-auth/self-register` | 从公开班级列表选择班级 + 姓名 + 自设 PIN + 自选难度 → 真的建号、入班、发令牌 |
| `PATCH /student-auth/me/english-level` | 学生自己改难度，身份只来自 Bearer |

**现行注册不是班级码制**：学生只从页面给出的班级列表中选择；班级和
英语难度是两个独立选择，页面不暗示某个班级对应某个难度。服务端只接受
注册列表里的班级 id，内部冒烟班不会出现在列表里，也不能靠伪造 id 注册。

**同班重名靠 `User.email` 的唯一索引**拦。email 是
`(classId, 归一后的姓名)` 的 sha256，所以双击撞到的是 P2002 而不是第二个
账号；用哈希而不是把姓名拼进去，是因为 email 会出现在日志与导出里。
事前那一次查重只是为了给一句人话的错误。

**换难度只写一个字段**，这就是全部设计：难度是**学生属性**，而他正在
上的那一天冻在答卷和 `DailyLessonCompletion` 里。于是 —— 还没开始的一天
按新难度挑场次，已经开始的一天保住它的文章、题目和词表，历史一条不动。
有一条测试直接读这个方法的源码，将来它长出第二个写就会红。也**不动**
`studentAuthVersion` —— 换个难度不是换凭据，不该把人踢下线。

**线上实测**（一个内部账号，一次走完）：选「雅思 · 简化版」注册 → 今天
是《The Bicycle Doctor》→ 开课前换成 O-Level → 今天变成《The Night
Market Cleans Up》→ 开始阅读 → 再换成「雅思 · 真题型」→ **今天仍然是
《The Night Market Cleans Up》、仍然是 `p1_olevel_20260831_sess`** →
登出重登，难度还是真题型。限流也在线上真的挡住了第 6 次尝试（429）。

**S12O 当时发现的「晚注册当天没有单词」运维缺口已经在 S12P 消除**：
当天课程的 `Paper.config.lessonWords` 是权威词表；新学生第一次明确开始
课程时，服务端幂等补齐自己的 `StudentWord`。`GET /lesson/today` 仍然
只读，只有开始课程这个明确动作会写入，脚本不再需要跟在每个新生后面重跑。

状态推进到 **`PILOT_READY_FOR_SELF_SIGNUP`**。**依然不是 PASS**，
阶段 13 依然一行未开始。

---

#### S12P —— 五档全周内容、手机 / iPad 与真实新生闭环上线

2026-09-01 按「今天先给小范围真实学生跑」的目标完成 staging 上线；生产
环境没有读取、写入或部署。代码提交 `960acfa`；staging API 从
`9e797f2f-318e-43c8-9a45-3d081f363090` 更新为
`20df78f5-39a5-4a5d-8e0d-fb9c96ea96cb`，学生端从
`397f9dc0-bf4a-4e46-bc13-bc8378be4b0c` 更新为
`f0522fe5-9e69-4810-9d31-06b4aa9c1dab`；`stg-web` 和 Postgres 服务
部署保持原样。

**注册与排课的现行真相**：注册页显示 9 个班级
（SGCE26W、SEC27W、OL26W、IAL27W、IAL27M、IAL26W、IAL26S2、
IAL26S1、IAL28S）和独立的 5 档难度（O-Level 基础 / 进阶 / 标准、
雅思轻量 / 真题型）。数据库核账是 9 个注册班、45 条班级难度配置；设置页
也显示同样五档。线上实测换到 O-Level 进阶后服务端保存成功，再换回雅思
真题型也成功；已经开始的当天仍保留原文章、题目和词表。

**本周内容**：2026-08-31 至 2026-09-04 共 5 天 × 5 档 = **25 篇
不同文章、250 道阅读题**，9 个班级每档每天都有独立 assignment 和
session（数据库为 250 / 250）。每篇 10 题，其中 6 道客观题服务端当场判，
4 道文字题等待老师；每档每天另带自己的教学词表。原创内容不复制往届真题。

**真实新生闭环**：新建验收学生 `cmti2nch600nqqgx9jc3m4bps`，班级
IAL28S，难度雅思真题型。手机宽 390 实走：注册信息可见 → 登录 →
《What the Ice Remembers》10 题 → 服务端当场判 6 题 → **21 张先教后测
的单词卡 → 21 道正式单词题** → 5/21、23.8 分 → 今日总结 2/2。
数据库最终为 21 个 StudentWord、1 份阅读答卷、1 份正式单词 attempt。
这证明「新生读完没有词」和「学 21 个只考 10 个」都不再存在。

**答题 / 结果 / 历史的一套界面**：手机按原文后题目纵向排；iPad 宽
1024 时历史逐题回顾实测为 444 px 原文 + 12 px 拖动条 + 521 px 题目，
原文默认展开，且页面没有横向溢出。结果与历史复用同一个 ResultView；
客观题只显示“正确答案”，等待人工判断的文字题只显示“参考答案”，不再
重复两行相同答案。iPad 上主要按钮实测 44–49 px 高。

**本轮先替学生解决的 10 个问题**：

1. 不知道班级码 —— 改成可见班级列表直接选；
2. 班级被误当成难度 —— 两个选择完全分开；
3. 原来的三档不够 —— 恢复五档并贯通注册、设置和排课；
4. 换难度怕丢历史 —— 历史不改，当天冻结，未来课程按新档；
5. 新生读完没有生词 —— 开课时按 paper 词表幂等补齐；
6. 教了 21 个却只考 10 个 —— 正式测试题数严格等于当天教学词数；
7. 学词阶段像考试 —— 先展示词、音标、词性、双语释义、英文定义和原句；
8. 首页三段不能直达 —— 阅读和单词完成前后都可直接点开，错题明确暂停；
9. 历史看不到原文 / 与答题长得不同 —— 统一成原文 + 题目的同一响应式界面；
10. 手机上挤、iPad 上浪费宽度 —— 手机单栏，iPad 可拖动双栏，安全区、
    17 px 基准字号、44 px 触控目标、键盘焦点与减少动画偏好一并处理。

额外可用性：生词本显示总数、到期数、掌握/学习/待开始、搜索和状态筛选；
自测可以选 5、10、20 或全部词，并明确“不计成绩”；错误与重试、保存中的
按钮锁、刷新恢复和重复点击保护均有自动化测试。错题本按用户决定继续暂停，
线上只显示“暂未开放”，不计入今天完成度，也没有继续开发旧错题交互。

**验证**：student-web 916、API 2153、旧 web 247 个测试全绿；三端
typecheck、学生端与 API build、`git diff --check` 均通过。staging API
health / ready 都是 200；手机 390×844 和 iPad 1024×768 的真实浏览器
走查通过。发布脚本五次均确认答卷指纹、非试点生词、复习流水、错题、申诉
和任务行未被改写。

现行状态是 **`PILOT_LIVE_FOR_SMALL_COHORT`**：小范围真实试点可以开始，
不是生产发布；生产仍须显式授权。阶段 13 仍未开始。

---

#### S12Q —— 点词翻译补库与变形释义纠正

2026-09-01 用户在《The Frog at MacRitchie》实测点击 `bumped`，页面返回
「本词典未收录这个词」。接口复现为 `{found:false, query:'bumped'}`。根因有
两层：staging 当时只有试点周临时播入的几百个词典条目；查询链直查失败后
也不会把规则过去式回退到真实存在的原形。

本轮做了两项修复：

1. 用仓库既有 ECDICT 导入器给 **staging** 补齐正式本地词典子集，只插不改
   不删。扫描 770,612 条，保留 94,572 条、补齐 8,520 个变形，最终
   `DictEntry` 为 **103,092** 条；学生作答、成绩、课程进度均未改动。
2. API 增加保守的 `-ed` 原形回退，只接受数据库里真实存在的候选，不调用
   联网翻译或 LLM，也不做会产生 `this→thi`、`morning→morn` 的宽泛拆词。
   ECDICT 自带的 `bumped` 中文是错误语境的「凸起的」；当变形词条没有任何
...
（这一段写于 12F，留着是当时的记录。**现行状态以上面 S12P 为准。**）

---

### 这一阶段没做什么

> · `ExamWordSheet` 已在 staging 使用 103,092 条 ECDICT 本地词典子集；真正
>   不存在的词仍会诚实显示「本词典未收录」，不调用联网翻译或 LLM；
> · S12P 已用新生账号在线走过「开始课程 → 21 张教学卡 → 21 道正式测试」，
>   不再把教学卡与题数对应关系列为未验证；
> · 账号设置已支持五档难度切换与改密码；更广的头像、通知等资料设置不是
>   本次小范围试点的阻塞项；
> · 用户先前冻结的错题本仍保持「暂未开放」，旧错题数据保留但本轮不开发；
> · **阶段 13 一行未开始**；
