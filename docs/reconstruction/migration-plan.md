...
| 测试六号 | — | — | — | — | — | — | **用户观察确认「没有问题」；非 Claude 观察** |
| 测试七号 | — | — | — | — | — | — | **用户观察确认；非 Claude 观察** |
| 测试八号 | — | — | — | — | — | — | **用户观察确认；非 Claude 观察** |

> **证据层级差异，如实记录**：六 / 七 / 八号的旧端主页由**用户本人**在浏览器中
> 登录并确认无问题，**Claude 没有读取过那三页**。旁证（不等于观察）：旧端公开的
> 成绩接口显示六号与八号各有 08-27 的 `graded 0/4` 提交、七号在「G12 无内容班」
> 且无提交 —— 与 API 的 `summary` / `no_content` 相符。是否满足 AC-06 由审查方裁定，
> 本文件不代为升格。
