...
> R0 · 2026-08-27 · 基线 `82b9cb0`
> **R0.1 修订** · 基线 `8303d1e` —— 新增部署 spike 阻断阶段、拆开主流程
> 迁移（阅读页独立）、修正回滚说明、灰度改独立开关与学生 ID 粒度。
> **计划文档，未实施。本轮不编码。**
...
- [ ] 记录基线：`8303d1e`，工作区干净，本地领先 `origin/main`
      （**不得 reset / rebase / 丢弃**）
- [ ] 打标签 `pre-reconstruction`
...
- [ ] `README.md`：删掉 "Out of scope: **student-facing UI**"
- [ ] `CLAUDE.md`：补上学生端产品定义与七步链
- [ ] 四份旧 PRD 加状态头（2.0 已被 4.0 取代；4.0 从「设计稿」改为
      「已实施至 RC1.1」；`student-auth-and-home.md` §6.3 已被
      `student-registration.md` 取代）
- [ ] `product-contract.md` 设为学生端唯一权威
...
- [ ] **S1** Railway 能否按路径前缀把一个域名分流到两个服务？
      不能则确定反代方案
- [ ] **S2** staging 上用**两个空白页面**把选定方案跑通
...
> 倒计时、交卷幂等。这些每一条都是学生每天在用的，任何一条坏掉都是 P1。
