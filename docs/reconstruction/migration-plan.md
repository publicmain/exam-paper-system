...
## 阶段 4B —— 新端 staging 集成　⬜ **需授权**

- [ ] 部署到保留的独立 staging 服务
- [ ] **配置并验证 CORS**（原 S6）—— 把新源加进 API 的 `CORS_ORIGINS`，
      验证带 `Authorization` 的预检通过；**还原时还原成原值、不是空值**
- [ ] 用 **staging 的八个账号**逐个走通登录 / 注册 / 改密码 / 退出
- [ ] **对着 staging 验教师重置**：重置后学生被踢回登录页并能重新注册

**授权类别**：**C2**（改 `CORS_ORIGINS`）+ **C3**（部署新端）。
其中改 `CORS_ORIGINS` **要动既有服务 `stg-api`** —— 超出「不碰既有服务」
的边界，需要单独批准。
**风险**：低（旧端不受影响）
**回滚**：删包
