...
## 阶段 12 —— 生词本与错题本　🔧 **12A–12D 本地完成 · 12E staging 实机验证完成 · 12F 账号就绪 · 12G/12H/12I 首次真人验收失败，服务端与界面两半已改完 · 12J 夹具重建并部署 · 12K 重建计划 · 12L 试点最小改动上线 · 12M 真内容进库、试点开跑 · 12O 学生自助注册与自选难度**（2026-08-31），**整阶段 `PILOT_READY_FOR_SELF_SIGNUP`**
...
**写了什么**（`apps/api/scripts/pilot/content/`，三档各两天）：
...
**范围收窄，明说**：合同里写的是五天，用户当场改成「全部等级全部做，
先做今天明天的」——所以只发了周一周二。周三之后要新写内容再发。
...
| 端点 | 干什么 |
| --- | --- |
| `POST /student-auth/self-register` | 班级码 + 姓名 + 自设 PIN + 自选难度 → 真的建号、入班、发令牌 |
| `PATCH /student-auth/me/english-level` | 学生自己改难度，身份只来自 Bearer |

**仍然是邀请制**：班级码就是邀请函，没有它谁也进不来。
...
**顺带发现的一件运维事**：单词是按**跑脚本那一刻的花名册**发的，所以
自助注册来的人如果晚于当天的脚本，当天没有单词（阅读不受影响）。
解法是顺序：**先让学生注册，再跑当天的脚本**；中途有人加入就把**当天**
的脚本再跑一遍。已写进开跑手册第 3 节。
...
（这一段写于 12F，留着是当时的记录。**现行状态以上面 12J 那一段为准。**）
...
> · **用户的 23 步人工走查一步都没开始** —— 账号已就绪、凭据已交接，
>   但今天那一课的第一个动作必须是用户本人的；
> · **教学卡先出只在库里验到，没在线上验到** —— `/vocab/lesson-cards`
>   在课程开始之前返回 0 张卡，而「开始今天的课程」是合同明令禁止我点的；
> · **账号设置扩展**仍是强制的后续任务，一行未开始；
> · **用户还没有再走一遍** —— 三半（服务端 / 界面 / 夹具）都做完了、
>   账号也重建并部署到位了，就差**用户本人重走那 23 步**；
> · **阶段 13 一行未开始**；
