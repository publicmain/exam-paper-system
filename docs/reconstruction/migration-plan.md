...
## 阶段 12 —— 生词本与错题本　🔧 **12A–12D 本地完成 · 12E staging 实机验证完成 · 12F 词典已补 · 验收账号已建但当天不干净**（2026-08-30），**整阶段 `AWAITING_USER_ACCEPTANCE`**
...
词典已补、账号已建、历史数据全对，**但当天还不干净**
（14 条复习流水落进了今天）—— 见本节末尾的 **12F** 小节。
...
> · **验收账号建出来了，但还不能用** —— 词典已补（+51）、
>   历史数据全对，但 **14 条复习流水落进了今天**，当天不干净。
>   代码已修 + 回归测试已加，**但按合同没有重跑**；
>   解阻 = 授权重跑一次夹具（会重新生成 PIN）；
