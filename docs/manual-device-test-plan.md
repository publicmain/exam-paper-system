...
> ## 📌 状态提示（2026-08-27 补）
>
> **本文档第 2 节让学生从 `/me` 进入 —— 那是旧的、新旧混杂的
> `apps/web`。** 它仍然有效，本文档描述的课程流程用例也仍然跑在那里。
>
> 但**学生端的 canonical 认证入口已经变了**：重建的新端
> `apps/student-web` 是一个**独立的源**，入口是它自己的 **`/login`**，
> 不是 `/me`。新端目前只有登录 / 注册 / 今天的课 / 账号四个页面，
> **还没有课程功能**（阶段 4A/4B）。
>
> 也就是说：
>
> - **要测认证** → 去新端的 `/login`
> - **要测课程流程** → 仍然用本文档的 `/me` 用例
>
> 本文档的历史课程流程用例**未改写**，原样保留。
> 权威口径见 `docs/reconstruction/product-contract.md`。
